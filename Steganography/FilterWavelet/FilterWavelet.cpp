...
CFilterWaveletApp::CFilterWaveletApp()
    : CPSPlugIn(sizeof(SFilterWaveletData), 113)
{
}
...
CFilterWaveletApp::FilterStart(void)
{
    ...
    SetTileWidth(m_sData->bWholeImage ? GetImageWidth()
                                      : min(m_sData->nTileX, GetImageWidth()));
    SetTileHeight(m_sData->bWholeImage
                      ? GetImageHeight()
                      : min(m_sData->nTileY, GetImageHeight()));
    // select the first tile again
    CPSPlugIn::IterationStart();

    // initialize the wavelet filter & current iteration
    waveletfilter::init(m_sData->nWaveletType,
                        m_sData->nWaveletPar,
                        m_sData->nWaveletLevel,
                        m_sData->nThreshType,
                        m_sData->nThreshSelector,
                        m_sData->dThreshCutoff);
    waveletfilter::reset();
    m_nIteration = 0;
}
...
CFilterWaveletApp::FilterContinue(void)
{
    ...
    if (m_nIteration == 0) {
        ...
        if (weight(in.size().rows()) != 1 || weight(in.size().cols()) != 1) {
            matrix<double> temp(1 << int(ceil(log2(in.size().rows()))),
                                1 << int(ceil(log2(in.size().cols()))));
            temp = 0;
            temp.copyfrom(in);
...
CFilterWaveletApp::ShowDialog(void)
{
    ...
    // tiling
    dlg.m_nTileX =
        (m_sData->nTileX == 0) ? GetSuggestedTileWidth() : m_sData->nTileX;
    dlg.m_nTileY =
        (m_sData->nTileY == 0) ? GetSuggestedTileHeight() : m_sData->nTileY;
...
CFilterWaveletApp::InitParameters()
{
    ...
    // tiling
    m_sData->nTileX =
        0; // special value - replaced with Photoshop suggestions on first call
