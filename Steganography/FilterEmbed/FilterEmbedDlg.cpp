...
BOOL CFilterEmbedDlg::OnInitDialog()
{
    ...
    m_nRawSize = m_pPSPlugIn->GetImageWidth() * m_pPSPlugIn->GetImageHeight() *
                 m_pPSPlugIn->GetPlanes();
    ComputeFileData();
    ComputeCodecData();
    ComputePunctureData();
    UpdateDisplay();
...
void CFilterEmbedDlg::OnOK()
{
    ...
    if (m_nEmbedRate < 1) {
        MessageBox(
            "Invalid bandwidth expansion rate.", NULL, MB_OK | MB_ICONWARNING);
        return;
    }
    if (m_bInterleave &&
        (m_dInterleaverDensity <= 0 || m_dInterleaverDensity > 1)) {
        MessageBox(
            "Density must be between 0 and 1.", NULL, MB_OK | MB_ICONWARNING);
        return;
    }
...
void CFilterEmbedDlg::OnComputeStrength()
{
    ...
    if (dlg.DoModal() == IDOK) {
        m_dEmbedStrength =
            round(10 * log10(dlg.m_dPower / double(255 * 255)), 0.1);
...
void CFilterEmbedDlg::UpdateDisplay()
{
    ...
    SetDlgItemText(IDC_INFO_FILESIZE,
                   (m_nSourceType == 3 && m_nFileSize > 0) ? sTemp : "");
    // capacity after variable-density interleaving
    const int nCapacity =
        m_bInterleave
            ? int(floor(m_nRawSize / m_nEmbedRate * m_dInterleaverDensity))
            : int(floor(m_nRawSize / double(m_nEmbedRate)));
    sTemp.Format("%d bits", nCapacity);
    SetDlgItemText(IDC_INFO_CAPACITY, sTemp);
    // code size information
    const int nInSize = m_nCodecIn;
    const int nOutSize =
        (m_nCodecOut == m_nPunctureIn) ? m_nPunctureOut : m_nCodecOut;
    sTemp.Format("(%d,%d)", nOutSize, nInSize);
    SetDlgItemText(IDC_INFO_CODESIZE, (nInSize > 0) ? sTemp : "");
    // number of usable bits
    const int nUsable = (nInSize > 0)
                            ? int(floor(nCapacity / double(nOutSize))) * nInSize
                            : nCapacity;
