...
CFilterLevelsApp::FilterContinue(void)
{
    ...
    // obtain neigborhood histograms for each threshold (ignoring 1-pixel
    // border)
    case 1: {
        // temporary variables
        vector<int> vi;
    ...
    if (IterationDone()) {
        ...
        case 1: {
            ...
            // overlap is of 2 pixels so that we can always ignore the 1-pixel
            // border
            SetTileOverlap(2);
        ...
        case 2: {
            ...
            for (i = 0; i < m_nLevels - 1; i++) {
                ...
                for (j = 1; j < 9; j++) // note: skipping zero because it does
                                        // not contribute to average
                {
            ...
            // (where the mode first reaches its maximum value from the black
            // side) note: original algorithm used median instead of mode
            int iBlack;
            viMedWeight.max(iBlack, true);
            m_dBlack = iBlack / double(m_nLevels - 1);

            // determine ideal white point
            // (where the average first reaches its maximum value from the white
            // side)
            int iWhite;
