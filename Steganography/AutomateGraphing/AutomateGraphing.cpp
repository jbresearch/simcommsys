...
CAutomateGraphingApp::~CAutomateGraphingApp() { delete m_sData; }
...
CAutomateGraphingApp::Process(void)
{
    ...
    for (double dStrength = m_sData->dStrengthMin;
         dStrength <= m_sData->dStrengthMax;
         dStrength += m_sData->dStrengthStep) {
        PlayeventFilterEmbed(/*embedding*/ 0,
                             1,
                             dStrength,
                             /*interleaver*/ true,
                             0,
                             ...
                             "");
        if (m_sData->bJpeg) {
            for (int nJpegQ = m_sData->nJpegMin; nJpegQ <= m_sData->nJpegMax;
                 nJpegQ += m_sData->nJpegStep) {
                // write row header
...
CAutomateGraphingApp::DoExtract(double dStrength)
{
    ...
    switch (m_sParameters.nFilterType) {
    case 0:
        PlayeventFilterATM(m_sParameters.uFilterSettings.sFilterATM.nRadius,
                           m_sParameters.uFilterSettings.sFilterATM.nAlpha,
                           true);
        break;
    case 1:
        PlayeventFilterAW(m_sParameters.uFilterSettings.sFilterAW.nRadius,
    ...
    PlayeventFilterExtract(/*embedding*/ 0,
                           1,
                           dStrength,
                           m_sData->bPresetStrength,
                           /*interleaver*/ true,
                           0,
                           1,
                           /*source*/ 2,
                           ...
                           "",
                           "",
                           /*compute*/ m_sData->bPrintBER,
                           m_sData->bPrintSNR,
                           m_sData->bPrintEstimate,
                           m_sData->bPrintChiSquare,
                           0);
}
