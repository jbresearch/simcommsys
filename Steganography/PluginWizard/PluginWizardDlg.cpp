...
void CPluginWizardDlg::OnOK()
{
    ...
    if (CreateDirectory(m_sPath + m_sNewName, NULL) == 0) {
        AfxMessageBox("Directory creation failed on " + m_sPath + m_sNewName +
    ...
    while (bWorking) {
        ...
        const CString sExtension =
            finder.GetFileName().Right(finder.GetFileName().GetLength() -
                                       finder.GetFileTitle().GetLength());
        ...
        char buf[buflen];
        for (fileIn.getline(buf, buflen); !fileIn.eof();
             fileIn.getline(buf, buflen)) {
            CString str = buf;
