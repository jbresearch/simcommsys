...
void testvector()
{
    ...
    cout << std::endl
         << "Test vector return by value:" << std::endl
         << std::endl;
...
void testboost_array()
{
    ...
    std::cout << std::endl
              << "Boost MultiArray Test:" << std::endl
              << std::endl;
    // Initial conditions
    // F = 0;
    F[0][0] = 1;
...
void display_array(boost::multi_array<T, 2>& A)
{
    ...
    for (typename array2_t::iterator i = A.begin(); i != A.end();
         ++i, std::cout << std::endl) {
        for (typename array1_t::iterator j = i->begin(); j != i->end();
             ++j, std::cout << "\t") {
            std::cout << *j;
        }
    }
...
void test128bit()
{
    ...
    for (int i = 0; i < 128 / 4; i++) {
        ...
        std::cout << std::setw(16)
                  << libbase::int64u((x >> 64) & 0xffffffffffffffffL);
