    ...
    int tau, double p, bool ins, bool del, bool sub, bool src, double Pr)
{
    ...
    cout << "Testing on an all-" << (src ? "one" : "zero")
         << " source:" << std::endl;
    cout << "   type:\t";
    if (ins) {
        cout << "insertions ";
    ...
    int tau, int n, double p, bool ins, bool del, bool sim, double Pr)
{
    ...
    const double error_I_dm = qids_utils::multiply_error_probability(
        qids_utils::compute_outofbounds_with(
            qids_utils::compute_drift_prob_exact, 1, Pi, Pd, I_dm, -1),
        tau);
    cout << "\t" << error_I_dm;
    if (Pi == Pd) {
    ...
    int m1_min, m1_max;
    qids_utils::compute_limits_with(
        qids_utils::compute_drift_prob_exact,
        1,
        Pi,
        Pd,
        qids_utils::divide_error_probability(Pr, tau),
        m1_min,
        m1_max);
    cout << "\t" << m1_min;
    cout << "\t" << m1_max;
    const double error_m1 = qids_utils::multiply_error_probability(
        qids_utils::compute_outofbounds_with(
            qids_utils::compute_drift_prob_exact, 1, Pi, Pd, m1_max, m1_min),
        tau);
    cout << "\t" << error_m1;
    ...
    int mn_min, mn_max;
    qids_utils::compute_limits_with(qids_utils::compute_drift_prob_exact,
                                    n,
                                    Pi,
                                    Pd,
                                    qids_utils::divide_error_probability(Pr, N),
                                    mn_min,
                                    mn_max);
    cout << "\t" << mn_min;
    cout << "\t" << mn_max;
    const double error_mn = qids_utils::multiply_error_probability(
        qids_utils::compute_outofbounds_with(
            qids_utils::compute_drift_prob_exact, n, Pi, Pd, mn_max, mn_min),
        N);
    cout << "\t" << error_mn;
...
void compute_drift_pdf(int tau, double p, bool ins, bool del, double Pr)
{
    ...
    for (int m = mtau_min; m <= mtau_max; m++) {
        const double prob =
            qids_utils::compute_drift_prob_exact(m, tau, Pi, Pd);
...
void test_receiver(int tau, double p, bool ins, bool del, bool sub, double Pr)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        const typename channel_t::metric_computer& computer =
            dynamic_cast<const typename channel_t::metric_computer&>(
                channel.get_computer());
        ...
        // compare results
        mse_trellis.insert((ptable_trellis - ptable_lattice).sumsq() /
                           (mtau_max - mtau_min + 1));
        mse_corridor.insert((ptable_corridor - ptable_lattice).sumsq() /
                            (mtau_max - mtau_min + 1));
    }
    // show results
    cout << p << "\t";
...
void test_precision(int tau, double p, bool ins, bool del, bool sub, double Pr)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        const typename channel_real_t::metric_computer& computer_real =
            dynamic_cast<const typename channel_real_t::metric_computer&>(
                channel_real.get_computer());
        ...
        const typename channel_reference_t::metric_computer&
            computer_reference = dynamic_cast<
                const typename channel_reference_t::metric_computer&>(
                channel_reference.get_computer());
        computer_reference.receive_trellis(tx, rx, ptable_trellis_reference);
        computer_reference.receive_lattice_corridor(
            tx, rx, ptable_corridor_reference);
        // compare results
        mse_trellis.insert(
            (vector<reference>(ptable_trellis_real) - ptable_trellis_reference)
                .sumsq() /
            (mtau_max - mtau_min + 1));
        mse_corridor.insert((vector<reference>(ptable_corridor_real) -
...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()(
        "blocksize,N", po::value<int>(), "block size in channel symbols");
    desc.add_options()(
        "codesize,n", po::value<int>(), "codeword size in channel symbols");
    desc.add_options()(
        "parameter,r",
        po::value<double>(),
    ...
    desc.add_options()(
        "ins,i", po::value<bool>()->default_value(true), "allow insertions");
    desc.add_options()(
        "del,d", po::value<bool>()->default_value(true), "allow deletions");
    desc.add_options()("sub,s",
                       po::value<bool>()->default_value(false),
                       "allow substitutions");
    desc.add_options()("exclusion,e",
                       po::value<double>()->default_value(1e-10),
    ...
    // determine timings for lattice and trellis receiver
        ...
        cout << "p\tmtau_min\tmtau_max\tm1_min\tm1_"
                "max\ttrellis\tlattice\tcorridor"
             << std::endl;
        for (double p = pstart; p <= pstop; p *= pmul) {
    ...
    // determine state-space limits
        ...
        const double pmul = pow(10.0, 1.0 / 10);
        cout << "p"
             << "\tI_dm\tp(e)\txmax_dm\tp(e)"
             << "\tI_exact\tp(e)\txmax_exact\tp(e)"
             << "\tm1_min\tm1_max\tp(e)"
             << "\tmn_min\tmn_max\tp(e)"
             << "\tmtau_min\tmtau_max\tp(e)";
        if (sim) {
            cout << "\txmax_est";
        }
        cout << std::endl;
