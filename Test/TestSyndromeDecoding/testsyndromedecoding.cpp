...
// ===========================================================================
//  TEST FIXTURE (This replaces creating RNG in every test)
// ===========================================================================
...
struct SimulationFixture {
    ...
    SimulationFixture() {
        // Set the seed ONCE here. 
        // This executes before the test suite starts.
        const int GLOBAL_SEED = 12345;
        rng.seed(GLOBAL_SEED);
        std::cout << ">>> Global RNG initialized with seed: " << GLOBAL_SEED << " <<<" << std::endl;
    }

    ~SimulationFixture() {
        // Cleanup if necessary (not needed for randgen)
    }
};
...
// This macro wraps all tests below in the SimulationFixture context.
// "rng" is now directly available in every test case.
BOOST_FIXTURE_TEST_SUITE(SyndromeTests, SimulationFixture)
...
std::cout << "TESTSYNDROMEDECODING: Modem Details: " << mdm->description() << std::endl;
    ...
    // Seed the channel
    // Pass the Global RNG from Fixture
    ...
    libbase::vector<libbase::gf2> received_codeword(n);
 ...
 */
BOOST_AUTO_TEST_CASE(gf2_random_message_zero_syndrome_loop)
{
    const libcomm::serializer_libcomm my_serializer_libcomm;
    std::cout << std::endl << "******* Boost Test 3a - Looping using zero syndrome decoding n times over GF2 *******" << std::endl; 
    ...
    double Ps_with_error = 0.02; // 2% errors
    ...
    std::cout << "\n--- RUN " << i << std::endl;
        ...
        libbase::vector<int> decoded_message(k);
        libbase::vector<int> generated_codeword(n);

        // Randomly generate message u with size k
        original_message = generate_random_message<int>(k, q, rng); 

        // Encode message to get error free codeword
        cdc->encode(original_message, generated_codeword);

#if DEBUG >= 2
    std::cout << "TESTSYNDROMEDECODING: original message: "
              << original_message << std::endl;
    std::cout << "TESTSYNDROMEDECODING: generated codeword: "
              << generated_codeword << std::endl;
#endif

        // ####################################################################
        // Adding noise to the generated codeword
        // ####################################################################
        ...
        // Decode (using default zero syndrome decoding)
        // Pass GLOBAL seed 
        cdc->seedfrom(rng);
    ...
    std::cout << "TESTSYNDROMEDECODING: Decoded message: "
        ...
        int num_errors = libbase::hamming(original_message, decoded_message);
...
{
    const libcomm::serializer_libcomm my_serializer_libcomm;
    std::cout << std::endl << "******* Boost Test 3b - Looping non-zero syndrome decode n times over GF2 *******" << std::endl; 
    ...
    for (int i = 0; i < TOTAL_RUNS; ++i) 
    {
        // Loop uses the global stream of random numbers.
#if DEBUG >= 2
    std::cout << "\n--- RUN " << i << std::endl;
#endif
    ...
    std::cout << "TESTSYNDROMEDECODING: Decoded message u (no error): "
        ...
        // Modulate -> Channel (Ps) -> Demodulate
        libbase::vector<libbase::gf2> modulated_codeword_p2(n);
        mdm->modulate(M, generated_codeword, modulated_codeword_p2); 
...
BOOST_AUTO_TEST_CASE(gf4_random_message_zero_syndrome_loop)
{
 ...
 */
    ...
    const libcomm::serializer_libcomm my_serializer_libcomm;
    std::cout << std::endl << "******* Boost Test 5a - Looping zero syndrome decoding n times over GF64 *******" << std::endl; 
    ...
    std::cout << "TESTSYNDROMEDECODING: generated codeword: "
        ...
        qsc_channel->set_parameter(Ps_with_error);
        qsc_channel->seedfrom(rng); // Pass the GLOBAL seed
 ...
 */
    ...
    const libcomm::serializer_libcomm my_serializer_libcomm;
    std::cout << std::endl << "******* Boost Test 5b - Looping non-zero syndrome decoding n times over GF64 *******" << std::endl; 
    ...
    std::cout << "TESTSYNDROMEDECODING: Decoded message u (no error): "
        ...
        qsc_channel->set_parameter(Ps_with_error);
        qsc_channel->seedfrom(rng); // Pss the GLOBAL seed
