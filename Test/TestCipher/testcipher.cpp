...
public:
    ...
    static void process()
    {
        ...
        for (iterator it = ciphers.begin(); it != ciphers.end(); it++) {
            std::list<BigInteger> partialDecryptions =
                my_testset.decryptCipher(grp, shares, *it);
        ...
        std::cout << my_testset.comparePlainTexts("plaintexts.txt",
                                                  "recoveredPlainText.txt")
                  << std::endl;
    }

    std::list<BigInteger>
    decryptCipher(libbase::group<BigInteger> grp,
                  std::list<libbase::keypair<BigInteger>> shares,
                  libbase::ciphertext<BigInteger> cipherText)
    {
        std::list<BigInteger> partialDecryptions;
        typedef
            typename std::list<libbase::keypair<BigInteger>>::iterator iterator;
        for (iterator it = shares.begin(); it != shares.end(); it++) {
            ...
            std::cout
                << libbase::equalitydisclog_zpschnorr<BigInteger>::verify(
                       grp,
                       libbase::elgamal<BigInteger>::createDecryptionProof(
                           grp, it->get_pri_key(), cipherText.get_gr()))
                << std::endl;
        }
        return partialDecryptions;
    }

    void createKeys(std::list<libbase::keypair<BigInteger>> shares,
                    std::list<BigInteger> publicShares,
                    libbase::group<BigInteger> grp)
    {
        for (int i = 0; i < 5; i++) {
            libbase::keypair<BigInteger> keyPair =
                libbase::keygenerator<BigInteger>::generateKeyShare(grp);

            libbase::knowdisclog_zpschnorr<BigInteger> proof =
                libbase::knowdisclog_zpschnorr<BigInteger>::createProof(
                    grp, keyPair.get_pri_key(), keyPair.get_pub_key());

            std::cout << "Proof is: ";
            std::cout
                << libbase::knowdisclog_zpschnorr<BigInteger>::verifyProof(
                       grp, proof);
    ...
    void createCiphers(int numberToCreate,
                       libbase::group<BigInteger> grp,
                       BigInteger publicKey)
    {
        ...
        for (int x = 0; x < numberToCreate; x++) {
            ...
            libbase::ciphertext<BigInteger> cipherText =
                libbase::elgamal<BigInteger>::encrypt(
                    plainText, grp, publicKey);
    ...
    bool comparePlainTexts(std::string originalFile, std::string recoveredFile)
    {
        ...
        while (getline(original, currentOriginalLine) &&
               getline(recovered, currentRecoveredLine)) {
...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()("type,t",
                       po::value<std::string>()->default_value("gmp"),
                       "big-integer type");
    po::variables_map vm;
