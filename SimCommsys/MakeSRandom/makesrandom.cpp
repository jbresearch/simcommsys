...
public:
    myvector(const int x = 0) : Base(x) {}
    T& operator()(int x) { return this->operator[](x); }
    const T& operator()(int x) const { return this->operator[](x); }
        ...
        for (attempt = 0; attempt < max_attempts && failed; attempt++) {
            ...
            for (int i = 0; i < tau && !failed; i++) {
                ...
                while (!failed) {
                    ...
                    mylist<pair<int>>::iterator ndx =
                        untried.getiteratorat(prng.ival(untried.size()));
...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()(
        "spread,s", po::value<int>(), "interleaver spread to start with");
    desc.add_options()("attempts,n",
