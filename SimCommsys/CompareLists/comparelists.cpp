...
public:
    coordinate() : x(0), y(0) {}
    explicit coordinate(const T x, const T y) : x(x), y(y) {}
...
int repeatability(const std::vector<T>& s1, const std::vector<T>& s2)
{
    ...
    // do the comparison
    int count = 0;
    for (typename std::list<T>::iterator i1 = t1.begin(); i1 != t1.end();
         i1++) {
             ...
             const bool mse)
{
    ...
    if (ham) {
        const int hd =
            libbase::hamming<T>(libbase::vector<T>(s1), libbase::vector<T>(s2));
...
int main(int argc, char* argv[])
{
    ...
    desc.add_options()("sequence1-file,i",
                       po::value<std::string>(),
                       "input file containing first sequence");
    desc.add_options()("sequence2-file,j",
                       po::value<std::string>(),
                       "input file containing second sequence");
    desc.add_options()("threshold,t",
                       po::value<double>()->default_value(0.5),
                       "threshold for Euclidian distance");
    desc.add_options()("coordinates,c",
                       po::bool_switch(),
                       "read elements as coordinate pairs");
    desc.add_options()(
        "hamming,h", po::bool_switch(), "compute Hamming distance");
    desc.add_options()(
        "levenshtein,l", po::bool_switch(), "compute Levenshtein distance");
    desc.add_options()("repeatability,r",
                       po::bool_switch(),
                       "compute number of common elements");
    desc.add_options()("mse,m",
                       po::bool_switch(),
    ...
    if (vm.count("help") || vm.count("sequence1-file") == 0 ||
        vm.count("sequence1-file") == 0 || (!ham && !lev && !rep && !mse)) {
        cout << desc << std::endl;
        return 0;
    }
