...
main(int argc, char* argv[])
{
    ...
    desc.add_options()("channel-file,h",
                       po::value<std::string>(),
    ...
    desc.add_options()("container,c",
                       po::value<std::string>()->default_value("matrix"),
    ...
    if (vm.count("help") || vm.count("system-file") == 0 ||
        vm.count("channel-file") == 0 || vm.count("parameter") == 0) {
    ...
    if (container == "matrix") {
        ...
        if (type == "int") {
            ...
            process<double, matrix>(
                systemfile, channelfile, parameter, softout);
        } else {
            std::cerr << "Unrecognized symbol type: " << type << std::endl;
            return 1;
        }
