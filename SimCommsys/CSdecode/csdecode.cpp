    ...
    const libbase::size_type<libbase::vector>& blocksize)
{
    ...
    const int length = tau + lookahead + eof_prior.size() - 1 - received.size();
#ifndef NDEBUG
    ...
    estimated_drift =
        libcomm::commsys_stream<S, libbase::vector, float>::estimate_drift(
            eof_post, offset);
    // Centralize posterior probabilities
    eof_post =
        libcomm::commsys_stream<S, libbase::vector, float>::centralize_pdf(
        ...
        std::ostream& sout = std::cout)
{
    ...
    std::shared_ptr<commsys> system = libcomm::loadfromfile<commsys>(fname);
    ...
    std::shared_ptr<commsys_stream> system_stream =
        std::dynamic_pointer_cast<commsys_stream>(system);
