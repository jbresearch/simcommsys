...
# create target file & set properties
touch "$dstpath/$dstfile"
svn add "$dstpath/$dstfile"
svn ps svn:keywords "Id Revision Author Date" "$dstpath/$dstfile"
svn ps svn:eol-style "LF" "$dstpath/$dstfile"
svn pd svn:executable "$dstpath/$dstfile"
svn ps svn:mime-type "text/x-c; charset=us-ascii" "$dstpath/$dstfile"
