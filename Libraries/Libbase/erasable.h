...
public:
    ...
    explicit erasable(int value = 0) : value(value), erased(false) {}
    explicit erasable(const symbol& value) : value(value), erased(false) {}
    // @}
    ...
    operator symbol() const { return get(); }
    ...
    static int elements() { return symbol::elements(); }
    // @}
};
...
public:
    /*! \name Constructors / Destructors */
    //! Principal constructor
    explicit erasable(int value = 0) : erased(false)
    {
        assert(value >= 0 && value <= 1);
        this->value = value & 1;
    }
    explicit erasable(const bool& value) : value(value), erased(false) {}
    // @}
