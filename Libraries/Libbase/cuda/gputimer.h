...
protected:
    ...
    void do_start() { cudaSafeCall(cudaEventRecord(event_start, stream)); }
    void do_stop() const { cudaSafeCall(cudaEventRecord(event_stop, stream)); }
...
public:
    ...
    //! Main constructor
    explicit gputimer(const std::string& name = "",
    ...
    void set_stream(const cudaStream_t stream) { this->stream = stream; }
    // @}
