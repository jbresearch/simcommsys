...
protected:
    ...
    size_t pitch
        __attribute__((aligned(8))); //!< Padded length of each row in bytes
...
public:
    ...
    __host__
#endif
    matrix() : data(NULL), pitch(0), rows(0), cols(0) {}
    // @}
...
matrix<T>::operator=(const libbase::matrix<T>& x)
{
    ...
    if (data != NULL) {
        for (int i = 0; i < rows; i++) {
            cudaSafeMemcpy(
                get_rowaddress(i), &x(i, 0), cols, cudaMemcpyHostToDevice);
        }
    }
...
inline matrix<T>::operator libbase::matrix<T>() const
{
    ...
    if (data != NULL) {
        for (int i = 0; i < rows; i++) {
            cudaSafeMemcpy(
                &x(i, 0), get_rowaddress(i), cols, cudaMemcpyDeviceToHost);
        }
    }
...
protected:
     ...
     */
    void init(const int m, const int n) { failwith("Not supported."); }
    // @}
public:
    /*! \name Constructors */
...
matrix_reference<T>::operator=(const libbase::matrix<T>& x)
{
    ...
    for (int i = 0; i < Base::rows; i++) {
        cudaSafeMemcpy(Base::get_rowaddress(i),
                       &x(i, 0),
                       Base::cols,
                       cudaMemcpyHostToDevice);
    }
...
public:
    ...
    __host__
#endif
    matrix_auto(const matrix_auto<T>& x) : isowner(false)
    {
        // do not invoke the base constructor, to avoid a deep copy
        Base::copyfrom(x);
#if DEBUG >= 2 && !defined(__CUDA_ARCH__)
