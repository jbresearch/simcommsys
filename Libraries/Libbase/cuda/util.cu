...
void cudaQueryDevices(std::ostream& sout)
{
    ...
    sout << "  CUDA Driver Version:\t"
         << cudaPrettyVersion(cudaGetDriverVersion()) << std::endl;
    sout << "  CUDA Runtime Version:\t"
    ...
    for (int i = 0; i < devices; i++) {
        ...
        sout << "Device " << i << ": \"" << cudaGetDeviceName(i) << "\""
             << std::endl;
        sout << "  CUDA Capability:\t"
             << cudaPrettyVersion(cudaGetComputeCapability(i)) << std::endl;
        sout << "  Global memory:\t" << cudaGetGlobalMem(i) << " bytes"
             << std::endl;
        sout << "  Multiprocessors:\t" << cudaGetMultiprocessorCount(i)
             << std::endl;
        sout << "  Total Cores:\t"
             << cudaGetMultiprocessorSize(i) * cudaGetMultiprocessorCount(i)
             << std::endl;
