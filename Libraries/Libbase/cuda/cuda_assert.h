...
#define cuda_assertalways(_Expression)                                         \
    (void)((!!(_Expression)) ||                                                \
           (cuda::reportassertionandfail(#_Expression, __FILE__, __LINE__),    \
            0))
...
#define cuda_failwith(_String)                                                 \
    cuda::reporterrorandfail(_String, __FILE__, __LINE__)
...
#endif
inline void
reportassertionandfail(const char* expression, const char* file, int line)
{
    ...
    printf("CUDA ERROR (%s line %d): assertion %s failed.\n",
           ...
           line,
           expression);
    // TODO: we really want to stop everything here, but there is no function to
    // do so.
    assert(0);
#endif
}
...
#endif
inline void
reporterrorandfail(const char* expression, const char* file, int line)
{
#ifndef __CUDA_ARCH__
