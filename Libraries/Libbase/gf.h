...
public:
     ...
     *
     * Addition within extensions of a field is the addition of the
     * corresponding coefficients in the polynomial representation. When the
     * field characteristic is 2 (ie. for extensions of a binary field),
     * addition of the coefficients is equivalent to an XOR operation.
     */
#ifdef __CUDACC__
    __device__
    __host__
     ...
     * corresponding coefficients in the polynomial representation. When the
     * field characteristic is 2 (ie. for extensions of a binary field),
     * subtraction of the coefficients is equivalent to an XOR operation, and
     ...
     * \param   x  Field element we want to multiply to this one (ie.
     * multiplicand).
     *
     * Multiplication within extensions of a field is the multiplication of the
     * polynomials representing the two values. This can be done by the usual
     * long-multiplication algorithm. Every time the result overflows, we need
     * to subtract the modular polynomial; for extensions of a binary field,
     * this is achieved by an XOR operation.
     *
     * [cf. Gladman, "A Specification for Rijndael, the AES Algorithm", 2003,
     * pp.3-4]
     */
#ifdef __CUDACC__
    __device__
     ...
     * - obtaining the logarithms of the two values, performing a subtraction,
     * and then computing the inverse logarithm
     *
    ...
    __host__
#endif
    gf& operator/=(const gf& x) { return *this *= x.inverse(); }
    /*!
