...
protected:
    ...
    /*! \brief Deallocates memory only (does not update internal size or
...
template <class T>
inline vector<T>::vector(const int n) : m_size(0), m_data(NULL)
{
    test_invariant();
    alloc(n);
    test_invariant();
...
template <class T>
inline vector<T>::vector(const vector<T>& x) : m_size(0), m_data(NULL)
{
    test_invariant();
...
template <class T>
template <class A>
inline vector<T>::vector(const vector<A>& x) : m_size(0), m_data(NULL)
{
    test_invariant();
    alloc(x.size().length());
...
template <class T>
template <class A>
inline vector<T>::vector(const std::vector<A>& x) : m_size(0), m_data(NULL)
{
    test_invariant();
    alloc(x.size());
    // avoid down-cast warnings in Win32
