...
template <class T>
inline void
matrix<T>::alloc(const int x, const int y)
{
    if (x == 0 && y == 0) {
...
template <class T>
template <class A>
inline matrix<T>::matrix(const matrix<A>& x) : m_size(0), m_data(NULL)
{
...
template <class T>
inline void
matrix<T>::init(const int m, const int n)
{
...
template <class T>
inline void
matrix<T>::insertrow(const vector<T>& v, const int i)
{
...
template <class T>
inline void
matrix<T>::insertcol(const vector<T>& v, const int j)
{
...
template <class T>
inline void
matrix<T>::extractrow(vector<T>& v, const int i) const
{
    assert(i >= 0 && i < m_size.rows());
    v.init(m_size.cols());
...
template <class T>
inline void
matrix<T>::extractcol(vector<T>& v, const int j) const
{
    assert(j >= 0 && j < m_size.cols());
    v.init(m_size.rows());
...
template <class T>
inline vector<T>
matrix<T>::extractrow(const int i) const
{
    vector<T> v;
    extractrow(v, i);
    return v;
...
template <class T>
inline vector<T>
matrix<T>::extractcol(const int j) const
{
    vector<T> v;
    extractcol(v, j);
    return v;
...
template <class T>
inline void
matrix<T>::serialize(std::istream& sin)
{
    for (int i = 0; i < m_size.rows(); i++) {
        for (int j = 0; j < m_size.cols(); j++) {
            sin >> m_data[i][j];
...
template <class T>
inline std::ostream&
operator<<(std::ostream& s, const matrix<T>& x)
{
    s << x.size() << std::endl;
    x.serialize(s);
    return s;
...
template <class T>
inline std::istream&
operator>>(std::istream& s, matrix<T>& x)
{
...
template <class T>
inline bool
matrix<T>::isequalto(const matrix<T>& x) const
{
    if (x.m_size != m_size) {
        return false;
    }
...
template <class T>
inline bool
matrix<T>::isnotequalto(const matrix<T>& x) const
{
    return !isequalto(x);
}
 ...
 */
template <class T>
inline libbase::matrix<T>
pow(const libbase::matrix<T>& A, int n)
{
...
private:
    ...
    masked_matrix<T>& operator*=(const matrix<T>& x) { return *this; }
