...
awfilter<T>::update(const libbase::matrix<T>& in)
{
    ...
    for (int i = 0; i < M; i++) {
        ...
        for (int j = 0; j < N; j++) {
            ...
            for (int ii = std::max(i - m_d, 0); ii <= std::min(i + m_d, M - 1);
                 ii++) {
                for (int jj = std::max(j - m_d, 0);
                     jj <= std::min(j + m_d, N - 1);
                     jj++) {
                    rv.insert(in(ii, jj));
                }
            }
                     ...
                     libbase::matrix<T>& out) const
{
    ...
    for (int i = 0; i < M; i++) {
        ...
        for (int j = 0; j < N; j++) {
            ...
            // compute result
            out(i, j) = T(mean + (std::max<double>(0, var - m_noise) /
                                  std::max<double>(var, m_noise)) *
                                     (in(i, j) - mean));
        }
    }
}
