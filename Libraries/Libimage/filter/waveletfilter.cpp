...
waveletfilter::update(const matrix<double>& in)
{
    ...
    case 1: {
        ...
        // unsupported type
    default: {
        std::cerr << "Unknown threshold selector (" << m_nThreshSelector << ")."
                  << std::endl;
    } break;
    }
}
...
waveletfilter::estimate()
{
    ...
    // % of coefficients
    case 0: {
        m_dThreshValue = m_vdCoefficient[int(
            round(m_vdCoefficient.size() * m_dThreshCutoff))];
    } break;
        // Visu
    case 1: {
        const double dSigma =
            m_vdCoefficient[m_vdCoefficient.size() / 2] / 0.6745;
        libbase::trace << "Estimated noise std = " << dSigma << " ("
                       << 20 * log10(dSigma) << "dB)" << std::endl;
...
waveletfilter::process(const matrix<double>& in, matrix<double>& out) const
{
    ...
    case 1: // soft thresholding
        ...
        break;
    default: // unknown
        std::cerr << "Unknown thresholding type (" << m_nThreshType << ")."
                  << std::endl;
        break;
    }
    const int count = out.mask(selection).size();
    const int n = in.size();
    libbase::trace << "Retained " << n - count << " coefficients ("
                   << 100.0 * (n - count) / n << "%)" << std::endl;
