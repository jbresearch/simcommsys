...
/*
 void wavelet::partial_titransform(vector<double>& a, vector<double>& hsr,
 vector<double>& hsl, vector<double>& lsr, vector<double>& lsl) const
 {
 const int n = a.size();
 // trap calls where n is too small
 if(n < 4)
...
void wavelet::init(const int type, const int par)
{
    ...
    case 2: {
        ...
        case 4: {
            const double f[] = {
                .000892313668, -.001629492013, -.007346166328, .016068943964,
                .026682300156, -.081266699680, -.056077313316, .415308407030,
                .782238930920, .434386056491,  -.066627474263, -.096220442034,
                .039334427123, .025082261845,  -.015211731527, -.005658286686,
                .003751436157, .001266561929,  -.000589020757, -.000259974552,
                .000062339034, .000031229876,  -.000003259680, -.000001784985};
            g.assign(f, sizeof(f) / sizeof(f[0]));
        } break;
        case 5: {
            const double f[] = {
                -.000212080863, .000358589677,  .002178236305, -.004159358782,
                -.010131117538, .023408156762,  .028168029062, -.091920010549,
                -.052043163216, .421566206729,  .774289603740, .437991626228,
                -.062035963906, -.105574208706, .041289208741, .032683574283,
                -.019761779012, -.009164231153, .006764185419, .002433373209,
                -.001662863769, -.000638131296, .000302259520, .000140541149,
                -.000041340484, -.000021315014, .000003734597, .000002063806,
                -.000000167408, -.000000095158};
            g.assign(f, sizeof(f) / sizeof(f[0]));
        } break;
            // Undefined
        default:
            std::cerr << "Undefined parameter (" << par
                      << ") for wavelet type (" << type << ")." << std::endl;
            return;
        }
    } break;
        // Daubechies
    case 3: {
        ...
        case 20: {
            const double f[] = {
                .026670057901,  .188176800078,  .527201188932,  .688459039454,
    ...
    case 4: {
        ...
        case 10: {
            const double f[] = {
                .001089170447,  .000135245020,  -.012220642630, -.002072363923,
    ...
    case 5: {
        const double f[] = {
            -.000062906118, .000343631905,  -.000453956620, -.000944897136,
            .002843834547,  .000708137504,  -.008839103409, .003153847056,
            .019687215010,  -.014853448005, -.035470398607, .038742619293,
            .055892523691,  -.077709750902, -.083928884366, .131971661417,
            .135084227129,  -.194450471766, -.263494802488, .201612161775,
            .635601059872,  .572797793211,  .250184129505,  .045799334111};
        g.assign(f, sizeof(f) / sizeof(f[0]));
    } break;
        // Battle-Lemarie
    case 6: {
        ...
        case 3: {
            const double f[] = {0.541736,     0.30683,      -0.035498,
                                -0.0778079,   0.0226846,    0.0297468,
                                -0.0121455,   -0.0127154,   0.00614143,
                                0.00579932,   -0.00307863,  -0.00274529,
                                0.00154624,   0.00133086,   -0.000780468,
                                -0.00065562,  0.000395946,  0.000326749,
                                -0.000201818, -0.000164264, 0.000103307};
            g.assign(f, sizeof(f) / sizeof(f[0]));
        } break;
        case 5: {
            const double f[] = {
