...
private: // FreeImage interface;
   ...
   double maxval(int bits) const { return double((1<<bits) - 1); };
   void unload();
...
public:
   ...
   format_type get_format() const { return format; };
   compression_type get_compression() const { return compression; };
   int get_quality() const { return quality; };
   void set_format(format_type format) { image::format = format; };
   void set_compression(compression_type compression) { image::compression = compression; };
   void set_quality(int quality) { image::quality = quality; };
