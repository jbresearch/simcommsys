...
const libbase::vcs awfilter_version("Adaptive Wiener Filter module (awfilter)",
      1.30);
...
template <class T> void awfilter<T>::process(const libbase::matrix<T>& in,
   ...
   for (int i = 0; i < M; i++)
      ...
      for (int j = 0; j < N; j++)
         ...
         // compute result
         out(i, j) = T(mean + (max<double> (0, var - m_noise) / max<double> (
               var, m_noise)) * (in(i, j) - mean));
         }
      }
   }
