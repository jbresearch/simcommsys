...
const libbase::vcs wavelet::version("Wavelet Transform Base module (wavelet)", 1.40);
...
void wavelet::partial_transform(const vector<double>& in, vector<double>& out, const int n) const
   {
...
void wavelet::partial_inverse(const vector<double>& in, vector<double>& out, const int n) const
   {
   assert(in.size() == out.size());
   assert(n <= in.size());
   // trap calls where n is too small
   ...
   switch(type)
      ...
      case 2: {
         switch(par)
            ...
            case 5: {
               const double f[] = { \
                  -.000212080863, .000358589677, .002178236305, \
            ...
            // Undefined
            default:
               cerr << "Undefined parameter (" << par << ") for wavelet type (" << type << ").\n";
               return;
            }
         } break;
      // Daubechies
...
void wavelet::transform(const vector<double>& in, vector<double>& out, const int level) const
   {
...
void wavelet::inverse(const vector<double>& in, vector<double>& out, const int level) const
   {
   assert(weight(in.size())==1);
   // resize the output vector if necessary
   out.init(in);
