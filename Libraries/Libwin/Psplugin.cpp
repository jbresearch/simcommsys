...
CPSPlugIn::DataAllocate()
{
    ...
    char** sHandle =
        m_pFilterRecord->handleProcs->newProc(m_nDataSize + sizeof(int));
    if (sHandle == NULL) {
...
CPSPlugIn::DataCheckVersion()
{
    ...
    char* sData =
        m_pFilterRecord->handleProcs->lockProc((char**)*m_pData, true);
    if (sData == NULL) {
...
CPSPlugIn::IterationDone() const
{
    ...
    return (inRect->top == 0 && inRect->left == 0 && inRect->bottom == 0 &&
            inRect->right == 0);
}
                               ...
                               const int nTotalIterations) const
{
    ...
    const int nPixelsDone =
        GetCurrentPlane() * GetImageWidth() * GetImageHeight() +
        GetCurrentCoordTop() * GetImageWidth() +
        GetCurrentCoordLeft() * GetCurrentTileHeight() + nTileDone;
    // display progress over all iterations
    DisplayTotalProgress(
        nPixelsDone, nPixelsTotal, nIteration, nTotalIterations);
}
...
CPSPlugIn::SetPixelMatrix(const libbase::matrix<double>& m)
{
    ...
    const int nXStop = GetCurrentCoordRight() == GetImageWidth()
                           ? GetCurrentTileWidth()
                           : GetCurrentTileWidth() - nTileOffset;
    const int nYStart = GetCurrentCoordTop() == 0 ? 0 : nTileOffset;
    const int nYStop = GetCurrentCoordBottom() == GetImageHeight()
                           ? GetCurrentTileHeight()
                           : GetCurrentTileHeight() - nTileOffset;
...
CPSPlugIn::CloseWriter(PIWriteDescriptor token)
{
    ...
    // close the write descriptor handle, and create new descriptor key/value
    // set
