...
/*
  ...
  finished off writing the high-level event playback functions for the
  streganography and binary suites.

   \version 1.02 (11 Nov 2002)
  bug fix: moved memory allocation for data block to a new private function,
  which is now called from Reload; added version checking for data block;
  removed de-allocation from Unload. [interim version - never used]

   \version 1.10 (12 Nov 2002)
   - based on Don Ashe's suggestions, and also following the Listener plugin as
  a sample, parameters are now only "stored" between calls through the scripting
  system. Thus, all data-block related code has been removed, and high-level
  routines for handling the scripting system have been added (similar to
  PSPlugIn). Derived classes are now responsible for creating and destroying
  their own data structures; this can be done in at least three ways: 1) by
  having the data variables members of the derived class, either separately or
  as a struct, 2) by allocating/deallocating memory in the class
  constructor/destructor respectively, or 3) by allocating/deallocating memory
  in the Reload/Unload calls, as specified by the photoshop API. It seems to me
  that option 2 allows for the greatest flexibility with minimal overhead.
   - added a new virtual function Execute, which is called when the Photoshop
  DoIt message is received. This encapsulates all that needs to be done at that
  stage, including reading/writing script parameters, showing the dialog, and
  calling Process.
   - changed the pData parameter in PluginMain from const void* to void*

   \version 1.11 (14 Nov 2002)
   - added Photoshop event playback for the following events: Open, Close,
  ConvertMode SelectState, SaveJPEG, Revert.
   - added support for handling alias and reference values (had only descriptors
  before).
  ...
  removed inclusion of PIDefines, which is part of the SDK sample code, and not
  the API itself.
  ...
  Compilations of automation classes were broken since v1.13 when PIDefines was
  removed; somehow this has not been detected earlier.
...
protected:
    ...
    void PutEnumerated(PIActionReference reference,
                       DescriptorClassID desiredClass,
                       DescriptorEnumTypeID type,
                       DescriptorEnumID value);
    void PutOffset(PIActionReference reference,
                   DescriptorClassID desiredClass,
                   int32 value);
    // high-level access - descriptor fill-in
    void
    PutInteger(PIActionDescriptor descriptor, DescriptorKeyID key, int data);
    void
    PutFloat(PIActionDescriptor descriptor, DescriptorKeyID key, double data);
    void
    PutBoolean(PIActionDescriptor descriptor, DescriptorKeyID key, bool data);
    void PutString(PIActionDescriptor descriptor,
                   DescriptorKeyID key,
                   const char* data);
    void
    PutAlias(PIActionDescriptor descriptor, DescriptorKeyID key, Handle alias);
    void PutReference(PIActionDescriptor descriptor,
                      DescriptorKeyID key,
                      PIActionReference reference);
    void PutEnumerated(PIActionDescriptor descriptor,
                       DescriptorKeyID key,
                       DescriptorEnumTypeID type,
                       DescriptorEnumID value);
    void PutObject(PIActionDescriptor descriptor,
                   DescriptorKeyID key,
                   DescriptorClassID type,
                   PIActionDescriptor value);
    // high-level access - descriptor read-out
    bool
    GetInteger(PIActionDescriptor descriptor, DescriptorKeyID key, int* data);
    bool
    GetFloat(PIActionDescriptor descriptor, DescriptorKeyID key, double* data);
    bool
    GetBoolean(PIActionDescriptor descriptor, DescriptorKeyID key, bool* data);
    bool
    GetString(PIActionDescriptor descriptor, DescriptorKeyID key, char* data);

    // high-level access - event playback
    PIActionDescriptor PlayEvent(DescriptorEventID event,
    ...
    void PlayeventFilterWavelet(int nWaveletType,
                                int nWaveletPar,
                                int nWaveletLevel,
                                int nThreshType,
                                int nThreshSelector,
    ...
    void PlayeventFilterEmbed(int nEmbedSeed,
                              ...
                              const char* sSource,
                              const char* sCodec);
    void PlayeventFilterExtract(int nEmbedSeed,
                                int nEmbedRate,
                                double dEmbedStrength,
    ...
    void
    PlayeventSaveJPEG(int nJpegQ, const char* sPathName, bool bCopy = false);
