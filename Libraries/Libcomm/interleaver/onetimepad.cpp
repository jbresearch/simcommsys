...
template <class real>
onetimepad<real>::onetimepad(const fsm& encoder,
                             const int tau,
                             const bool terminated,
                             const bool renewable)
    : terminated(terminated), renewable(renewable),
      encoder(std::dynamic_pointer_cast<fsm>(encoder.clone()))
{
...
onetimepad<real>::onetimepad(const onetimepad& x)
    : terminated(x.terminated), renewable(x.renewable),
      encoder(std::dynamic_pointer_cast<fsm>(x.encoder->clone())), pad(x.pad),
      r(x.r)
{
}
