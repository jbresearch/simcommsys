...
inline double anneal_interleaver::energy_function(const int i, const int j)
   ...
   if(sets < 2 && type < 15)
      ...
      const double c1 = (r <= tau) ? (r * PI/2) : (r * (PI/2 - 2*acos(tau/r)));
      ...
      switch(type)
         {
         case 2:
            return (d_in > 5*m) ? 0 : (5*m - d_in + 1)/sqrt(double(d_out));
         case 3:
            return (d_in > 5*m || d_out > 5*m) ? 0 : (5*m - d_in + 1)*(5*m - d_out + 1);
         case 4:
         ...
         case 14:
            ...
            const int l_out = (d_out % p == 0) ? (tau-d_out) : (tau-min(lut(0,i),lut(0,j)));
            // finally compute delta value
            return log(double(l_in + l_out));
            }
         default:
            std::cerr << "FATAL ERROR (anneal_interleaver): Energy function type [" << type << "] for 1 set not implemented\n";
            exit(1);
         }
      }
   else
      ...
      switch(type)
         ...
         default:
            std::cerr << "FATAL ERROR (anneal_interleaver): Energy function type [" << type << "] for " << sets << " set(s) not implemented\n";
            exit(1);
         }
      }
