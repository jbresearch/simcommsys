...
/*!
   ...
   - Initial version, implementing Forward-Backward Algorithm for a HMM. This is
     based on the paper by Davey & McKay, "Watermark Codes: Reliable communication
     over Insertion/Deletion channels", Trans. IT, 47(2), Feb 2001.
   - Implements algorithm on a single block; in the case of Davey's Watermark codes,
     each block is N elements of n-bit in length, and is the size of the sparsifier's
     output for a single LDPC codeword. Typical values of n were 5,6,7. With watermark
     codes, N was typically in the range 500-1000. For other examples of LDPC codes,
     Davey used N up to about 16000.

   \version 1.10 (29-30 Oct 2007)
   - made P() and Q() functions protected (rather then private) and virtual, as these
     are meant to be defined by derived classes. Also made these pure virtual, to ensure
     they do get defined by a derived class.
   - made the destructor virtual since this class now has virtual functions.
   - added xmax
   - changed definition of received vector to be a vector of signal-space symbols; the
     actual type is a template parameter. This change affects the definitions of decode(),
     Q(), and word_forward() and work_backward().
