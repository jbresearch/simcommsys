...
#if DEBUG >= 2
    std::cerr << "DEBUG (commsys_stream): old offset = " << oldoffset
              << std::endl;
    std::cerr << "DEBUG (commsys_stream): frame drift = " << drift << std::endl;
    std::cerr << "DEBUG (commsys_stream): new offset = " << newoffset
              << std::endl;
    std::cerr << "DEBUG (commsys_stream): existing segment = "
    ...
    libbase::size_type<C>& offset) const
{
    ...
    if (eof_post.size() == 0) // this is the first frame
    {
        ...
        sof_prior = eof_post;
        // Initialize as drift pdf after transmitting one frame, given sof
        // priors (offset gets updated and sof_prior gets resized as needed)
...
#define FINITE_TYPE_SEQ (bool)GF_TYPE_SEQ

#define ADD_ERASABLE(r, x, type) (type)(erasable<type>)
...
#define INSTANTIATE(r, args)                                                                     \
    ...
    const serializer commsys_stream<BOOST_PP_SEQ_ENUM(args)>::shelper(                           \
        "commsys",                                                                               \
        "commsys_stream<" BOOST_PP_STRINGIZE(BOOST_PP_SEQ_ELEM(0, args)) "," BOOST_PP_STRINGIZE( \
            BOOST_PP_SEQ_ELEM(                                                                   \
                1, args)) "," BOOST_PP_STRINGIZE(BOOST_PP_SEQ_ELEM(2,                            \
                                                                   args)) ">",                   \
