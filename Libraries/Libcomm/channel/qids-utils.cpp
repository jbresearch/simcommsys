...
#if DEBUG >= 3
    std::cerr << "DEBUG (qids-utils): [pdf-davey] m = " << m
              << ", sigma = " << sigma << ", this_p = " << this_p << "."
              << std::endl;
#endif
    return this_p;
}
 ...
 */
double
qids_utils::compute_drift_prob_exact_0(int m, int T, double Pi, double Pd)
{
    assert(Pi > 0 && Pd > 0);
...
#if DEBUG >= 3
    std::cerr << "DEBUG (qids-utils): [pdf-exact] m = " << m
              << ", p_j0 = " << pj << std::endl;
#endif
    // main computation
    ...
    for (int j = j0 + 1; j <= T; j++) {
        ...
        this_p += exp(pj);
#if DEBUG >= 3
        std::cerr << "DEBUG (qids-utils): [pdf-exact] j = " << j
                  << ", p_j = " << pj << ", this_p = " << this_p << std::endl;
#endif
    }
    return this_p;
}
 ...
 */
double
qids_utils::compute_drift_prob_exact_1(int m, int T, double Pi, double Pd)
{
    assert(Pi > 0 && Pd == 0);
    typedef long double myreal;
 ...
 */
double
qids_utils::compute_drift_prob_exact_2(int m, int T, double Pi, double Pd)
{
    assert(Pi == 0 && Pd > 0);
    typedef long double myreal;
 ...
 */
double
qids_utils::compute_drift_prob_exact_3(int m, int T, double Pi, double Pd)
{
    assert(Pi == 0 && Pd == 0);
    return (m == 0) ? 1 : 0;
}
...
qids_utils::compute_drift_prob_exact(int m, int T, double Pi, double Pd)
{
    ...
    validate(Pd, Pi);
#if DEBUG >= 3
    std::cerr << "DEBUG (qids-utils): compute_drift_prob_exact(" << m << ","
              << T << "," << Pi << "," << Pd << ")" << std::endl;
#endif
    // caching of results
    ...
    if (Pi > 0 && Pd > 0) { // handle non-degenerate case: Pi > 0, Pd > 0
        this_p = compute_drift_prob_exact_0(m, T, Pi, Pd);
    } else if (Pi > 0) { // handle degenerate case 1: Pi > 0, Pd = 0
        this_p = compute_drift_prob_exact_1(m, T, Pi, Pd);
    } else if (Pd > 0) { // handle degenerate case 2: Pd > 0, Pi = 0
        this_p = compute_drift_prob_exact_2(m, T, Pi, Pd);
    } else { // handle degenerate case 3: Pi = Pd = 0
        this_p = compute_drift_prob_exact_3(m, T, Pi, Pd);
    }
#if DEBUG >= 3
    std::cerr << "DEBUG (qids-utils): [pdf-exact] this_p = " << this_p
              << std::endl;
#endif
 ...
 *
 * \f[ I = \left\lceil \frac{ \log{P_r} - \log T }{ \log P_i } \right\rceil - 1
 * \f] where \f$ P_r \f$ is an arbitrary probability of having a block of size
...
qids_utils::compute_I(int T, double Pi, double Pr)
{
    ...
    I = std::max(I, 1);
#if DEBUG >= 2
    std::cerr << "DEBUG (qids-utils): for T = " << T << ", I = " << I
              << std::endl;
#endif
    return I;
}
...
#if DEBUG >= 3
    std::cerr << "DEBUG (qids-utils): [davey] Q(" << factor
              << ") = " << libbase::Q(factor) << std::endl;
#endif
    // main computation
    const double p = Pi;
