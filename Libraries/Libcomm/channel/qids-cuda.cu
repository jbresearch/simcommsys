                                                ...
                                                array1r_t& ptable) const
{
    ...
    cuda::receive_trellis_kernel<G, real>
        <<<1, 1>>>(*this, dev_tx, dev_rx, dev_ptable);
    // return the result
                                                ...
                                                array1r_t& ptable) const
{
    ...
    dev_rx = rx;
    // call the kernel with a copy of this object
    cuda::receive_lattice_kernel<G, real>
        <<<1, 1>>>(*this, dev_tx, dev_rx, dev_ptable);
    // return the result
    ptable = array1r_t(dev_ptable);
}
    ...
    const array1g_t& tx, const array1g_t& rx, array1r_t& ptable) const
{
    ...
    dev_rx = rx;
    // call the kernel with a copy of this object
    cuda::receive_lattice_corridor_kernel<G, real>
        <<<1, 1>>>(*this, dev_tx, dev_rx, dev_ptable);
    // return the result
    ptable = array1r_t(dev_ptable);
}
...
#define INSTANTIATE_FUNC(r, args)                                              \
    template __global__ void receive_trellis_kernel(                           \
        const typename libcomm::qids<BOOST_PP_SEQ_ENUM(args)>::metric_computer \
            object,                                                            \
...
BOOST_PP_SEQ_FOR_EACH_PRODUCT(INSTANTIATE_FUNC,
                              (SYMBOL_TYPE_SEQ)(REAL_TYPE_SEQ))
...
#define INSTANTIATE_CLASS(r, args) template class qids<BOOST_PP_SEQ_ENUM(args)>;

BOOST_PP_SEQ_FOR_EACH_PRODUCT(INSTANTIATE_CLASS,
                              (SYMBOL_TYPE_SEQ)(REAL_TYPE_SEQ))
