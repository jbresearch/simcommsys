...
// 2 - Show Markov state, error sequence, and tx/rx vectors during transmission
// process 3 - Show tx/rx vectors and posterior probabilities from receive
// process 4 - Show lattice rows as they are computed in receive process
#ifndef NDEBUG
#    undef DEBUG
#    define DEBUG 1
#endif
                                          ...
                                          array1r_t& ptable1) const
{
    ...
    const int mT_min =
        last ? (Zmin - S0) : std::max(-int(std::ceil(T / 2.0)), Zmin - S0);
#if DEBUG >= 3
    ...
    // *** compute second row (i = 1)
    {
        ...
        if (delta0 == 0) // last bit of previous codeword not deleted
        {
            for (int j = jmin; j <= jmax; j++) {
                ...
                // implicit free delete with no transmission at end of last
                // codeword
                if (last && j - i < mT_max &&
                    j + S0 == n) { // (j)-(i-1) <= mT_max
                    temp += F1[j];
    ...
    if (last) // no correction factors on last codeword
        ...
        for (int x = Zmin; x < Zmax; x++) {
            ...
            // last tx bit deleted
            if (j >= 0 &&
                j <= rho) // (j)-(n-1) <= mT_max must be true (x < Zmax)
            {
...
void bpmr<real>::generate_state_sequence(const int tau)
{
    ...
    for (int i = 0; i < tau; i++) {
        ...
        } else if (Zprev == Zmin) { // lower limit
            if (p < Pi) {
                Z(i) = Zprev + 1;
            } else {
                Z(i) = Zprev;
 ...
 */
template <class real>
bpmr<real>::bpmr(const bool varyPd,
                 const bool varyPi,
                 const bool varyPs,
                 const bool varyPsi)
    : varyPd(varyPd), varyPi(varyPi), varyPs(varyPs), varyPsi(varyPsi), Zmax(1),
      Zmin(0), fixedPd(0), fixedPi(0), fixedPs(0), fixedPsi(0)
{
