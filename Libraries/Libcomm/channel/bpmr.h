 ...
 * pp.35-45, Jan. 2011
 * URL:
 * http://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=5676449&isnumber=5676431
 *
...
private:
    ...
    bool varyPs;  //!< Flag to indicate that \f$ P_s \f$ should change with
                  //!< parameter
    bool varyPsi; //!< Flag to indicate that \f$ P_{si} \f$ should change with
                  //!< parameter
    int Zmax; //!< Maximum value of Markov state (equal to K-1 in Iyengar paper)
    int Zmin; //!< Minimum value of Markov state (0 for Iyengar model, otherwise
              //!< negative)
    ...
    double fixedPs;  //!< Value to use when \f$ P_s \f$ does not change with
                     //!< parameter
    double fixedPsi; //!< Value to use when \f$ P_{si} \f$ does not change with
                     //!< parameter
                     // @}
public:
    ...
    private:
        real get_transmission_coefficient(int Z) const
        {
            ...
            if (Zmin == Zmax) { // degenerate case with one state
                ...
                // only insertion or transmission were possible
                return real(1) - Pi;
            } else if (Z == Zmax) { // Z == mT_max
                // only deletion or transmission were possible
                return real(1) - Pd;
            } else { // mT_min < Z < mT_max
                // general case: insertion, deletion, or transmission
                return real(1) - Pi - Pd;
            }
        }
    ...
    public:
        ...
        void receive(const array1b_t& tx,
                     const array1b_t& rx,
...
public:
    ...
    bpmr(const bool varyPd = true,
         const bool varyPi = true,
         const bool varyPs = false,
         const bool varyPsi = false);
    // @}
     ...
     */
    void set_pr(const double Pr) { assert(Pr > 0 && Pr < 1); }
    /*!
    ...
    bool is_statespace_fixed() const { return true; }
    ...
    const typename channel_insdel<bool, real>::metric_computer&
    get_computer() const
    {
        return computer;
    }
