...
__global__
void receive_trellis_kernel(const libcomm::bsid::metric_computer object,
...
__global__
void receive_lattice_kernel(const libcomm::bsid::metric_computer object,
      const cuda::bitfield tx, const cuda::vector_reference<bool> rx,
      cuda::vector_reference<libcomm::bsid::real> ptable)
   {
...
void bsid::metric_computer::receive_trellis(const bitfield& tx,
   ...
   cuda::receive_trellis_kernel<<<1,1>>>(*this, dev_tx, dev_rx, dev_ptable);
...
void bsid::metric_computer::receive_lattice(const bitfield& tx,
   ...
   dev_tx = tx;
   // call the kernel with a copy of this object
   cuda::receive_lattice_kernel<<<1,1>>>(*this, dev_tx, dev_rx, dev_ptable);
   cudaSafeThreadSynchronize();
   // return the result
   ptable = array1r_t(dev_ptable);
   }
