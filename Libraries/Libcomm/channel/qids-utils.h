...
private:
    ...
    public:
        pair(const int& a, const int& b) : a(a), b(b) {}
    };
    ...
    static double
    compute_drift_prob_exact_0(int m, int T, double Pi, double Pd);
    static double
    compute_drift_prob_exact_1(int m, int T, double Pi, double Pd);
    static double
    compute_drift_prob_exact_2(int m, int T, double Pi, double Pd);
    static double
    compute_drift_prob_exact_3(int m, int T, double Pi, double Pd);
    // @}
public:
    ...
    /*!
     * \brief Computes the probability of drift 'm' after transmitting 'T'
     * symbols using the supplied algorithm and drift pdf at start of
     * transmission.
     *
     * The final drift pdf is obtained by convolving the expected pdf for a
     * known start of frame position with the actual start of frame
     * distribution.
     */
    template <typename F>
    static double
    compute_drift_prob_with(const F& compute_pdf,
                            int m,
                            int T,
                            double Pi,
                            double Pd,
     ...
     */
    template <typename F>
    static double compute_outofbounds_with(
        const F& compute_pdf, int T, double Pi, double Pd, int upper, int lower)
    {
        // sanity checks
        assert(T > 0);
        validate(Pd, Pi);
     ...
     */
    template <typename F>
    static void compute_limits_with(const F& compute_pdf,
                                    int T,
                                    double Pi,
                                    double Pd,
                                    double Pr,
    ...
    /*!
     * \brief Determine maximum absolute drift at the end of a frame of \f$ T
     * \f$ symbols, using the supplied algorithm for computing the drift pdf.
     *
     * The drift range is chosen such that the probability of having the drift
     * after transmitting \f$ T \f$ symbols being greater than \f$ \pm x_{max}
     * \f$ is less than an arbitrary value \f$ P_r \f$.
     */
    template <typename F>
    static int compute_xmax_with(
        const F& compute_pdf, int T, double Pi, double Pd, double Pr)
    {
        // sanity checks
        assert(T > 0);
        validate(Pd, Pi);
...
#if DEBUG >= 2
        std::cerr << "DEBUG (qids): [computed] for T = " << T
                  << ", xmax = " << xmax << "." << std::endl;
        std::cerr << "DEBUG (qids): [davey] for T = " << T
                  << ", xmax = " << compute_xmax_davey(T, Pi, Pd, Pr) << "."
                  << std::endl;
#endif
        return xmax;
    }
    ...
    /*!
     * \brief Determine maximum drift at the end of a frame of 'T' symbols,
     * given the supplied drift pdf at start of transmission.
     */
    static int compute_xmax(
        int T,
        double Pi,
        double Pd,
        double Pr,
...
#if DEBUG >= 3
        std::cerr << "DEBUG (qids): [exact] for T = " << T
                  << ", xmax = " << xmax << "." << std::endl;
#endif
        return xmax;
    }
