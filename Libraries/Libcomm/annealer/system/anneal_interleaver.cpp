...
anneal_interleaver::anneal_interleaver(
    const int sets, const int tau, const int m, const int type, const bool term)
{
...
inline double anneal_interleaver::energy_function(const int i, const int j)
{
    if (sets < 2 && type < 15) {
        ...
        case 2:
            return (d_in > 5 * m) ? 0
                                  : (5 * m - d_in + 1) / sqrt(double(d_out));
        case 3:
            return (d_in > 5 * m || d_out > 5 * m)
                       ? 0
                       : (5 * m - d_in + 1) * (5 * m - d_out + 1);
        case 4:
