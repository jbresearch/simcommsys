...
anneal_puncturing::anneal_puncturing(const char* fname,
                                     const int tau,
                                     const int s)
{
    ...
    if (file == NULL) {
        std::cerr << "FATAL ERROR (anneal_puncturing): Cannot open "
                     "contribution file ("
                  << fname << ")." << std::endl;
        exit(1);
    }
    ...
    fclose(file);
    // initialise the puncturing pattern as odd-even (and transmitting all data
    // bits)
    {
        pattern.init(s, tau);
        for (int i = 0; i < s; i++) {
