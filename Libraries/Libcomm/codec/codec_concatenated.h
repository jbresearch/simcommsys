...
protected:
    ...
    {
        // NOTE: app applies only to *first* codec, but we need to decode last
        // first
...
public:
    ...
    void seedfrom(libbase::random& r)
    {
        ...
        // Seed codecs
        for (typename codec_list_t::iterator it = codec_list.begin();
             it != codec_list.end();
             it++) {
            (*it)->seedfrom(r);
        }
        ...
        // Seed mappers
        for (typename mapper_list_t::iterator it = mapper_list.begin();
             it != mapper_list.end();
             it++) {
            (*it)->seedfrom(r);
        }
