...
private:
    ...
    std::shared_ptr<fsm>
        encoder;    //!< Encoder object (same for all parity sequences)
    ...
    libbase::vector<array2d_t>
        R; //!< A priori intrinsic encoder-output statistics (interleaved)
    libbase::vector<array2d_t> ra; //!< A priori extrinsic source statistics
    libbase::vector<array1d_t> ss; //!< Holder for start-state probabilities
                                   //!< (used with circular trellises)
    libbase::vector<array1d_t> se; //!< Holder for end-state probabilities (used
                                   //!< with circular trellises)
    // @}
    /*! \name Internal functions */
    //! Memory allocator (for internal use only)
    void allocate();
    // wrapping functions
    static void work_extrinsic(const array2d_t& ra,
                               const array2d_t& ri,
                               const array2d_t& r,
                               array2d_t& re);
    void
    bcjr_wrap(const int set, const array2d_t& ra, array2d_t& ri, array2d_t& re);
...
protected:
    ...
    int enc_parity() const { return enc_outputs() - enc_inputs(); }
...
public:
    ...
    int num_inputs() const { return enc_symbols(); }
    int num_outputs() const { return enc_symbols(); }
