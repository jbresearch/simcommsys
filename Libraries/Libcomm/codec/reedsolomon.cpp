...
reedsolomon<GF_q>::softdecode(array1vd_t& ri, array1vd_t& ro)
{
    ...
    // we use the PGZ algorithm for decoding General BCH codes (note that RS
    // codes are narrow-sense BCH codes. see
    ...
    if (dec_success) {
        ...
        if (GF_q(0) != det) {
            // We can now read off the coefficients of the error locator
            // polynomial
            ...
            // the X_i indicate the error positions of the received word as
            // follows: Suppose that errors happened at positions, j_1, j_2,
            // ..,j_w then X_i=\alpha^{j_i}.
#if DEBUG >= 2
            std::cout
                << std::endl
            ...
            while ((rootsfound < t) && (counter < this->length_n)) {
                ...
                if (tmp_val == GF_q(0)) {
                    // we have found a root, \beta=\alpha^s of the error locator
                    // polynomial, eg say (X_1 \beta +1)=0 then
                    // X_1=\frac{1}{\beta}=\alpha^{-s} Now X_1=\alpha^{j_1} this
                    // means that j_1=-s and as we only deal with powers between
            ...
            if (rootsfound != 0) {
                ...
                for (int rows = 0; rows < this->dim_pchk; rows++) {
                    for (int cols = 0; cols < rootsfound; cols++) {
                        error_mat(rows, cols) =
                            this->pchk_matrix(rows, error_pos(cols));
                    }
                }
                for (int rows = 0; rows < this->dim_pchk; rows++) {
...
reedsolomon<GF_q>::checkParams(int length, int dim)
{
    ...
    // only consider RS codes that are narrow-sense BCH codes (ie n=q-1) for the
    // moment
...
reedsolomon<GF_q>::init()
{
     ...
     *
     *
     * We consider the generator polynomial
     * g(x)=(x-1)(x-alpha)(x-alpha^2)...(x-alpha^(2t-1)) where t>=1 Write
     * g(x)=g_0+g_1*x+g_2*x^2+...+g_{2t-1}*x^{2t} Set n-k=deg(g) where n=q-1, ie
     * k=q-1-(deg(g))=q-1-2t=q-(2t+1) Consider the following information
     * polynomial of the form u(x)=u_0+u_1x+...+u_(k-1)x^(k-1) with u_i in
     * GF(2^m) then there are clearly 2^(k*m) such polynomials.
     *
     * Using u(x) defined as above, we can compute v(x)= u(x)*g(x) where
     * deg(v)<=deg(u)+deg(g)=k-1+(n-k)=n-1
     *
     * The coefficients of v(x) yield the code generated by g(x).
     *
     * Its generator matrix is clearly given by the following matrix:
     *
     *   [g_0   g_1 ... g_{n-k-2} g_{n-k-1}  g_{n-k}       0     0    0    ...
     * 0    ] [ 0    g_0 ... g_{n-k-3} g_{n-k-2} g_{n-k-1}  g_{n-k}   0    0 ...
     * 0    ] [ 0     0  ... ...  0    ] G=[ . ] [ . ] [ . ] [ 0     0  ... g_0
     * g_1  g_2 ... g_{n-k} ]
     *
     * Using the Singleton-bound, it is clear that
     * d<=n-k+1=(q-1)-(q-(2t+1))+1=2t-1
     *
     * Note that we can turn G into an extended Reed-Solomon code by simply
     * adding another column that is the sum of all the previous columns. let
     * p=g_0+g_1+...+g_{n-k} then the generator matrix for the extended RS code
     * is simply:
     *
     *   [g_0   g_1 ... g_{n-k-2} g_{n-k-1}  g_{n-k}       0     0    0    ...
     * 0    p] [ 0    g_0 ... g_{n-k-3} g_{n-k-2} g_{n-k-1}  g_{n-k}   0    0
     * ...  0    p] [ 0     0  ... ...  0    p] G=[ . p] [ . p] [ . p] [ 0     0
     * ...                                   g_0   g_1  g_2 ... g_{n-k} p]
     *
     *
     * Looking at v(x)=u(x)g(x) we see that v(y)=0 when
     * y={1,alpha,...,alpha^(2t-1)}, ie the roots of g(x).
     *
     * Consider the following matrix now:
     *
     *   [1    alpha           alpha^2             alpha^3      ... alpha^(n-3)
     * alpha^(n-1)     ] [1   alpha^2        (alpha^2)^2         (alpha^3)^2 ...
     * (alpha^(n-3))^2       (alpha^(n-1)^2    ] H=[. ... ]
     *   [.                                                     ... ]
     *   [.                                                     ... ] [1
     * alpha^(2t-1)  (alpha^2)^(2t-1)  (alpha^3)^(2t-1)   ...
     * (alpha^(n-3))^(2t-1)  (alpha^(n-1))^(2t-1)]
     *
     * Then it is clear that H*v=0
     *
     * The BCH bound now guarantees that the minimum distance of the code is
     * d>=2t-1. So n-k>=2t-1. Now the dimension of the code for which H is the
     * parity check matrix is k=q-2t+1 and by the Singleton bound
     * d<=n-k+1=(q-1)-(q-2t+1)+1=2t-1 and hence d=2t-1.
     *
     * we can construct the parity check matrix of an extended RS code with
     * [q,k,d+1] as follows (see Lin-p240):
     *
     *    [1    alpha           alpha^2             alpha^3      ... alpha^(n-3)
     * alpha^(n-1)      1] [1   alpha^2        (alpha^2)^2         (alpha^3)^2
     * ...   (alpha^(n-3))^2       (alpha^(n-1)^2     0] H'=[. ... 0]
     *    [.                                                     ... 0]
     *    [.                                                     ... 0] [1
     * alpha^(2t-1)  (alpha^2)^(2t-1)  (alpha^3)^(2t-1)   ...
     * (alpha^(n-3))^(2t-1)  (alpha^(n-1))^(2t-1) 0]
     *
     * We will use the parity check matrix construction to obtain the required
     * parity check matrix for the code and then use standard matrix
     * manipulation to obtain the generator matrix for the code. in other words
     * we use the following equalities if G=(P|I) then H=(I|-P^t) where P^t is
     * the transpose of the matrix P. since
     * G*H^t=(P|I)*(I|-P^t)^t=P-(-P^t)^t)=P-P=0
     *
     */
    ...
    // 2 in binary is 10 which represents the generating element alpha
    int alpha = GF_q(2);
    // this is alpha^0=1;
