 ...
 * \author S Wesemeyer
 * This class will construct a Reed-Solomon code over F_{q} of length n and
 * dimension k Note that n is either q or q-1 and 1<k<n-1 The Berlekamp
 * algorithm is used to decode any received word.
 *
 */
template <class GF_q>
...
protected:
     ...
     * \param[out] encoded Sequence of output (encoded) symbols, one per
     * timestep
     *
...
public:
    ...
    //! Number of valid input combinations, ie the number of bits that make up a
    //! symbol in the information set.
    int num_inputs() const { return GF_q::elements(); }

    //! Number of valid output combinations, ie the number of bits that make up
    //! a symbol used by the encoded version.
     ...
     * This method will construct the parity check and generator matrix for the
     * RS code under investigation.
     */
    void init();
