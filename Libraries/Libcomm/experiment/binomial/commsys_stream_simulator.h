...
private:
    ...
    std::list<array1i_t>
        source;         //!< List of message sequences in order of transmission
    ...
    libbase::size_type<libbase::vector>
        estimated_drift; //!< Estimated drift in last decoded frame
    std::list<array1i_t> act_bdry_drift; //!< Actual channel drift at codeword
                                         //!< boundaries of each received frame
    std::list<int>
        actual_drift; //!< Actual channel drift at end of each received frame
    int drift_error; //!< Cumulative error in channel drift estimation at end of
                     //!< last decoded frame
    std::shared_ptr<commsys_stream<S, libbase::vector, real>>
        sys_enc;        //!< Copy of the commsys object for encoder operations
    ...
    hard_decision<libbase::vector, double, int>
        hd_functor; //!< Hard-decision box
                    // @}
...
protected:
    ...
    {
        return dynamic_cast<commsys_stream<S, libbase::vector, real>&>(
            *this->sys);
    }
    // @}
...
public:
    ...
    commsys_stream_simulator() : stream_mode(stream_mode_open), N(0)
    {
        reset();
    }
