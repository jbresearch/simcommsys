    ...
    libbase::vector<double>& result)
{
    ...
    for (int left = length - received.size(); left > 0 || source.empty();) {
        ...
        const array1i_t source_next =
            this->src->generate_sequence(sys_dec.input_block_size());
        ...
        if (rc) {
            ...
            // (decoder-side)
            const array1i_t act_drift =
                sys_dec.gettxchan_stream().get_drift(boundary_pos);
...
#if DEBUG >= 2
        std::cerr
            << "DEBUG (commsys_stream_simulator): Actual received frame = "
            << rho << std::endl;
...
#ifndef NDEBUG
            std::cerr << "DEBUG (commsys_stream_simulator): EOF drift = "
                      << drift << " < mtau_min" << std::endl;
#endif
            eof_prior(0) = 1;
        } else if (drift + offset >= eof_prior.size()) { // drift > mtau_max
#ifndef NDEBUG
            std::cerr << "DEBUG (commsys_stream_simulator): EOF drift = "
                      << drift << " > mtau_max" << std::endl;
#endif
    ...
    for (int iter_modem = 0; iter_modem < sys_dec.sys_iter(); iter_modem++) {
        ...
        if (rc) {
            ...
            // get actual drift at codeword boundary positions to compare
            // against
            assert(!act_bdry_drift.empty());
            const array1i_t act_drift = act_bdry_drift.front();
            // Tell user what we're doing
#if DEBUG >= 4
            std::cerr << "DEBUG (commsys_stream_simulator): act bdry drift = "
                      << act_drift << std::endl;
            std::cerr << "DEBUG (commsys_stream_simulator): est bdry drift = "
                      << est_drift << std::endl;
#endif
            // accumulate results
    ...
    estimated_drift = commsys_stream<S, libbase::vector, real>::estimate_drift(
        eof_post, offset);
    // Centralize posterior probabilities
    eof_post = commsys_stream<S, libbase::vector, real>::centralize_pdf(
        eof_post, estimated_drift);
    // Tell user what we're doing
#if DEBUG >= 3
