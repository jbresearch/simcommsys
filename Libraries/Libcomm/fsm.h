...
public:
     ...
     *
     * Left-most register positions (ie. those closest to the input junction)
     * are represented by lower index positions, and get lower-order positions
     * within the integer representation.
     */
     ...
     *
     * \note This function has to be called once by each function
     * re-implementing it.
     */
     ...
     * re-implementing it.
     */
    virtual void reset(const array1i_t& state) { reset(); }
     ...
     */
    void resetcircular() { resetcircular(state(), N); }
    // @}
     ...
     * re-implementing it.
     */
    virtual void advance(array1i_t& input) { ++N; }
    ...
    int num_states() const { return int(pow(num_symbols(), mem_elements())); }
