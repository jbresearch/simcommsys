...
void map_reshape<dbl>::dotransform(const array2i_t& in, array2i_t& out) const
{
    ...
    libbase::trace << out.size().rows() << "×" << out.size().cols()
                   << std::endl;
#endif
                                   ...
                                   array2vd_t& pout) const
{
    ...
    libbase::trace << pout.size().rows() << "×" << pout.size().cols()
                   << std::endl;
#endif
    // Transform encoder output posteriors to blockmodem priors (row-major
    // order)
    int ii = 0, jj = 0;
