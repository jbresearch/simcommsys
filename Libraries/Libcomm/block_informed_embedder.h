...
#ifndef __block_informed_embedder_h
#define __block_informed_embedder_h
...
/*!
 * \brief   Blockwise Informed Data Embedder/Extractor Common Interface.
 * \author  Johann Briffa
 *
 * Class defines common interface for block_informed_embedder classes.
 */

template <class S,
          template <class> class C = libbase::vector,
          class dbl = double>
class basic_block_informed_embedder : public blockprocess
{
public:
...
protected:
    ...
    //! \copydoc embed()
    virtual void
    doembed(const int N, const C<int>& data, const C<S>& host, C<S>& stego) = 0;
    //! \copydoc extract()
    virtual void doextract(const channel<S, C>& chan,
                           const C<S>& rx,
                           const C<S>& reference,
...
public:
    ...
    //! Virtual destructor
    virtual ~basic_block_informed_embedder() {}
    // @}

    /*! \name Block informed embedder operations */
    /*!
     ...
     * values
     * \param[in]  reference Reference value (estimate of the host value)
     ...
     * schemes such as (key-dependent) SSIS.
     */
    void extract(const channel<S, C>& chan,
                 const C<S>& rx,
                 const C<S>& reference,
                 C<array1d_t>& ptable);
    // @}
...
/*!
 * \brief   Blockwise Informed Data Embedder/Extractor Base.
 * \author  Johann Briffa
 *
 * Class defines base interface for block_informed_embedder classes.
 */

template <class S,
          template <class> class C = libbase::vector,
          class dbl = double>
class block_informed_embedder : public basic_block_informed_embedder<S, C, dbl>,
                                public libbase::serializable
{
public:
    // Serialization Support
    DECLARE_BASE_SERIALIZER(block_informed_embedder)
};
