...
private:
    ...
    double threshold; //!< threshold for convergence (interpretation depends on
                      //!< mode)
    enum {
        mode_relative_error = 0, //!< converge when error margin as a fraction
                                 //!< of result mean is less than threshold
        mode_absolute_error, //!< converge when error margin (ie its absolute
                             //!< value) is less than threshold
        mode_accumulated_result, //!< converge when absolute accumulated result
                                 //!< (ie result mean x sample count) is more
                                 //!< than threshold
        mode_undefined
...
public:
          ...
          tupdate("montecarlo_update")
    {
        ...
        std::shared_ptr<libbase::functor> fgetparameter(
            new libbase::specificfunctor<montecarlo>(
                this, &libcomm::montecarlo::slave_getparameter));
    ...
    virtual ~montecarlo() { tupdate.stop(); }
    // @}
    ...
    //! Set target error margin as a fraction of result mean (eg 0.10 => 10% of
    //! mean)
    ...
    {
        assertalways(threshold > 0);
        libbase::trace << "DEBUG (montecarlo): setting threshold for "
                          "accumulated result to "
                       << threshold << std::endl;
        this->threshold = threshold;
