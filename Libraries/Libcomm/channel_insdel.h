...
#    ifdef __CUDACC__
        //! Receiver interface
        __device__
        virtual real receive(const cuda::vector_reference<S>& tx,
                             const cuda::vector_reference<S>& rx) const = 0;
        //! Batch receiver interface
        __device__
        virtual void receive(const cuda::vector_reference<S>& tx,
                             const cuda::vector_reference<S>& rx,
...
#    endif
        ...
        virtual real receive(const S& tx, const array1s_t& rx) const = 0;
        //! Receiver interface - given transmitted sequence
        virtual real receive(const array1s_t& tx,
                             const array1s_t& rx) const = 0;
...
public:
    ...
    /*!
     * \brief Get the actual channel drift at time 't' of last transmitted
     * frame.
     */
    virtual int get_drift(int t) const = 0;
    /*!
     * \brief Get the actual channel drift at a set of times 't' of last
     * transmitted frame.
     */
