...
public:
...
#endif
        std::shared_ptr<fsm> encoder_copy =
    ...
    array1i_t state() const { return fsm::convert_state(ps); }
    void reset() { ps = 0; }
    void reset(const array1i_t& state) { ps = fsm::convert_state(state); }
    ...
    int mem_elements() const { return nu; }
    ...
    // Description
    std::string description() const { return "Cached " + base_description; }
