...
public:
    ...
    double measure(qubit&) const override {
        failwith("fake_position_observable does not support qubit measurement.");
        return 0;
    }

    double measure(entangled_qubit_pair&, int) const override {
        failwith("fake_position_observable does not support entangled qubit pair measurement.");
        return 0;
    }

    double measure(epr_beam&, int) const override {
        failwith("fake_position_observable does not support EPR beam measurement.");
        return 0;
    }
};
