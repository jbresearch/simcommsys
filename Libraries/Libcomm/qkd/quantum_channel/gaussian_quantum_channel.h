...
private:
    ...
    double VA; // Modulation variance of Alice that will be set by a qkd_commsys object.
    double noise_mean; // Chosen mean of the ND to generate the noise. This is a serialized parameter.
    std::mt19937 gen;

protected:
    double compute_VN() // Calculates variance VN to generate the noisy coherent states.
    {
...
public:
    ...
    gaussian_quantum_channel()
        : noise_transmittance(0.0),
          noise_detector_eff(0.0),
          SNR(0.0),
          VA(0.0),
          noise_mean(0.0),
          gen() {}
