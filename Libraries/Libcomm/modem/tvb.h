...
public:
    ...
    enum codebook_t {
        codebook_sparse =
            0,           //!< sparse codes of length 'n', as in DM construction
        codebook_random, //!< randomly-constructed codebooks of length 'n',
                         //!< update every frame
...
private:
    ...
    int globalstore_limit;  //!< fba memory threshold in MiB for global storage,
                            //!< if applicable
    int lookahead; //!< Number of codewords to look ahead when stream decoding
    // @}
    /*! \name Internally-used objects */
    std::shared_ptr<channel_insdel<sig, real2>>
        mychan;                 //!< bound channel object
    mutable libbase::randgen r; //!< for construction and random application of
                                //!< codebooks and marker sequence
    mutable array2vs_t encoding_table;   //!< per-frame encoding table
    mutable bool changed_encoding_table; //!< flag indicating encoding table has
...
protected:
    ...
    void fill_encoding_table(array2vs_t& encoding_table,
                             const int offset,
                             const int length) const;
    void demodulate_wrapper(const channel<sig>& chan,
                            const array1s_t& rx,
                            const int lookahead,
                            const array1d_t& sof_prior,
                            const array1d_t& eof_prior,
...
private:
    ...
    // Setup function
    void
    init(const channel<sig>& chan, const array1d_t& sof_pdf, const int offset);
    ...
    // Other utilities
    void checkforchanges(int m1_min,
                         int m1_max,
                         int mn_min,
                         int mn_max,
                         int mtau_min,
...
public:
    ...
    explicit tvb(const int n = 2,
                 const int q = 2,
                 const double th_inner = 0,
                 const double th_outer = 0,
                 const int tp_states = 0)
        : q(q), marker_type(marker_zero), codebook_type(codebook_random),
    ...
    tvb(const tvb& x)
        : stream_modulator<sig>(x), parametric(x), q(x.q),
          ...
          Pr(x.Pr), flags(x.flags), storage_type(x.storage_type),
          globalstore_limit(x.globalstore_limit), lookahead(x.lookahead),
          r(x.r), encoding_table(x.encoding_table),
          changed_encoding_table(x.changed_encoding_table),
          mtau_min(x.mtau_min), mtau_max(x.mtau_max)
    {
        if (x.mychan) {
            mychan = std::dynamic_pointer_cast<channel_insdel<sig, real2>>(
                x.mychan->clone());
        }
    }
    // @}
    ...
    void set_parameter(const double x) { set_thresholds(real(x), real(x)); }
    ...
    int num_codebooks() const { return codebook_tables.size().rows(); }
    ...
    double energy() const { return get_avg_codeword_length(); }
    ...
    {
        return libbase::size_type<libbase::vector>(get_avg_codeword_length() *
                                                   lookahead);
    }
    double get_suggested_exclusion(void) const { return Pr; }
