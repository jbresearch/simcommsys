                               ...
                               int mtau_max) const
{
    ...
    if (last_m1_min != m1_min || last_m1_max != m1_max) {
        std::cerr << "DEBUG (dminner): m1_min = " << m1_min
    ...
    if (last_mn_min != mn_min || last_mn_max != mn_max) {
        std::cerr << "DEBUG (dminner): mn_min = " << mn_min
    ...
    if (last_mtau_min != mtau_min || last_mtau_max != mtau_max) {
        std::cerr << "DEBUG (dminner): mtau_min = " << mtau_min
        ...
        for (int d = 0; d < q; d++) {
            ...
            for (int x1 = x1min; x1 <= x1max; x1++) {
                ...
                for (int x2 = x2min; x2 <= x2max; x2++) {
                    ...
                    const real R = real(mychan.get_computer().receive(
                        t, r.extract(n * i + x1, x2 - x1 + n)));
                            ...
                            array1vd_t& ptable)
{
    ...
    mychan.compute_limits(
        n, qids_utils::divide_error_probability(Pr, N), mn_min, mn_max);
    int m1_min, m1_max;
    mychan.compute_limits(
...
dminner<real>::serialize(std::ostream& sout) const
{
    ...
    sout << "# marker type (0=random, 1=zero, 2=symbol-alternating, "
            "3=mod-vectors)"
         << std::endl;
