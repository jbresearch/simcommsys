    ...
    if (N > 0) {
        ...
        if (lookahead) {
            ...
            for (int offset = N, left = lookahead; left > 0;
                ...
                copy.fill_frame_marker_sequence(
                    frame_marker_sequence, offset, length);
            }
        }
    }
}
                                            ...
                                            array1vd_t& ptable)
{
    ...
    demodulate_wrapper(chan,
                       ...
                       sof_post,
                       eof_post,
                       libbase::size_type<libbase::vector>(mtau_max));
}
...
#ifndef NDEBUG
    std::cerr << "DEBUG (marker): offset = " << offset
              << ", mtau_min = " << fba.get_mtau_min() << "." << std::endl;
#endif
                                               ...
                                               int mtau_max) const
{
    ...
    if (last_m1_min != m1_min || last_m1_max != m1_max) {
        std::cerr << "DEBUG (marker): m1_min = " << m1_min
    ...
    if (last_mtau_min != mtau_min || last_mtau_max != mtau_max) {
        std::cerr << "DEBUG (marker): mtau_min = " << mtau_min
...
BOOST_PP_SEQ_FOR_EACH_PRODUCT(
    INSTANTIATE, (ALL_SYMBOL_TYPE_SEQ)(REAL_TYPE_SEQ)(REAL2_TYPE_SEQ))
