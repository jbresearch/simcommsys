        ...
        // Generate codebook
        {
            ...
            for (int d = 0; d < q; d++) {
                for (bool ready = false; !ready;) {
                    for (int s = 0; s < n; s++) {
                        const_cast<sig&>(codebook_tables(0, d)(s)) =
                            sig(this->r.ival(field_utils<sig>::elements()));
                    }
                    // this entry should be distinct from earlier entries
                    ready = true;
                    for (int dd = 0; dd < d; dd++) {
                        if (codebook_tables(0, dd).isequalto(
                                codebook_tables(0, d))) {
                            ready = false;
                            break;
                        }
                                         ...
                                         array1vd_t& ptable)
{
    ...
    demodulate_wrapper(chan,
                       ...
                       sof_post,
                       eof_post,
                       libbase::size_type<libbase::vector>(-mtau_min));
}
...
#ifndef NDEBUG
    std::cerr << "DEBUG (tvb): offset = " << offset
              << ", mtau_min = " << mtau_min << "." << std::endl;
#endif
                                               ...
                                               const int offset)
{
    ...
    std::cerr << "max at " << libbase::index_of_max(sof_prior) - offset
              << std::endl;
    std::cerr << "eof_prior = " << eof_prior << std::endl;
    std::cerr << "max at " << libbase::index_of_max(eof_prior) - offset
              << std::endl;
#endif
#if DEBUG >= 5
    ...
    fba_ptr->decode(*this,
                    ...
                    eof_post_r,
                    offset);
    // In cases with lookahead, re-compute EOF posterior at actual frame
    // boundary
                                 ...
                                 const int offset)
{
    ...
    mychan =
        std::dynamic_pointer_cast<channel_insdel<sig, real2>>(chan.clone());
    ...
    int mn_min, mn_max;
    mychan->compute_limits(
        nmax, qids_utils::divide_error_probability(Pr, N), mn_min, mn_max);
    int m1_min, m1_max;
    mychan->compute_limits(
        1, qids_utils::divide_error_probability(Pr, tau), m1_min, m1_max);
    ...
    const int required =
        fba_type::get_memory_required(N, q, mtau_min, mtau_max, mn_min, mn_max);
    switch (storage_type) {
    case storage_local:
        globalstore = false;
        break;
    ...
    if (!fba_ptr || globalstore != last_globalstore) {
        ...
        const bool thresholding =
            th_inner > real(0) || th_outer > real(0) || tp_states > 0;
        fba_ptr = fba2_factory<sig, real, real2>::get_instance(
...
void tvb<sig, real, real2>::validatecodebook() const
{
    ...
    for (int i = 0; i < num_codebooks(); i++) {
        for (int d = 0; d < q; d++) {
            ...
            for (int dd = 0; dd < d; dd++) {
                assertalways(
                    codebook_tables(i, dd).isnotequalto(codebook_tables(i, d)));
            }
        }
    }
}
                                            ...
                                            int mtau_max) const
{
    ...
    if (last_m1_min != m1_min || last_m1_max != m1_max) {
        std::cerr << "DEBUG (tvb): m1_min = " << m1_min
                  << ", m1_max = " << m1_max << std::endl;
        last_m1_min = m1_min;
        last_m1_max = m1_max;
    }
    ...
    if (last_mn_min != mn_min || last_mn_max != mn_max) {
        std::cerr << "DEBUG (tvb): mn_min = " << mn_min
                  << ", mn_max = " << mn_max << std::endl;
        last_mn_min = mn_min;
        last_mn_max = mn_max;
    }
    ...
    if (last_mtau_min != mtau_min || last_mtau_max != mtau_max) {
        std::cerr << "DEBUG (tvb): mtau_min = " << mtau_min
                  << ", mtau_max = " << mtau_max << std::endl;
...
std::string tvb<sig, real, real2>::description() const
{
    ...
    case codebook_user_sequential:
        sout << codebook_name << " codebook [" << codebook_tables.size().rows()
             << ", sequential]";
        break;
...
std::ostream& tvb<sig, real, real2>::serialize(std::ostream& sout) const
{
    ...
    sout << "# Storage mode for gamma (0=local, 1=global, 2=conditional)"
         << std::endl;
...
std::istream& tvb<sig, real, real2>::serialize(std::istream& sin)
{
    ...
    if (version >= 2 && version <= 9) {
        ...
        if (!norm) {
            std::cerr << "WARNING: no-normalization not supported."
                      << std::endl;
        }
        if (!batch) {
            std::cerr << "WARNING: non-batch interface not supported."
                      << std::endl;
        }
    }
    ...
    case marker_user_random: {
        ...
        assertalways(codebook_type == codebook_user_sequential ||
