...
private:
    ...
    bool user_threshold; //!< flag indicating that thresholds are supplied by
                         //!< user
...
private:
    ...
    const int demodulate(const bool& signal,
                         const libbase::vector<double>& app) const
    {
        failwith("Function should not be used.");
        return 0;
    }
...
private:
    ...
    void checkforchanges(int m1_min,
                         ...
                         int mtau_min,
                         int mtau_max) const;
    void work_results(const array1b_t& r,
                      array1vr_t& ptable,
                      const int mtau_min,
                      const int mtau_max,
                      const int mn_min,
...
public:
    ...
    int get_symbolsize(int i) const { return n; }
    int num_codebooks() const { return codebook.size().rows(); }
