...
private:
   ...
   mutable cuda::matrix_auto<cuda::vector<sig> > encoding_table; //!< Local copy of per-frame encoding table
   typename libcomm::qids<sig>::metric_computer computer; //!< Channel object for computing receiver metric
   // @}
public:
...
*/
   ...
   void R(int d, int i, const cuda::vector_reference<sig>& r,
         cuda::vector_reference<qids_real>& ptable) const
      {
