...
private:
    ...
    mutable cuda::vector_auto<sig>
        encoding_table; //!< Local copy of per-frame encoding table, flattened
    mutable cuda::vector_auto<int>
        cw_start; //!< Start of segment in encoding table for codeword 'i'
    mutable cuda::vector_auto<int> cw_length; //!< Length of codeword 'i'
    typename libcomm::qids<sig, real2>::metric_computer
        computer; //!< Channel object for computing receiver metric
                  // @}
public:
     ...
     */
    void
    init(const typename libcomm::channel_insdel<sig, real2>::metric_computer&
             computer)
    {
        this->computer = dynamic_cast<
            const typename libcomm::qids<sig, real2>::metric_computer&>(
            computer);
#if DEBUG >= 2
        ...
        std::cerr << "Rtable = " << libbase::matrix<real2>(computer.Rtable)
                  << std::endl;
#endif
    }
...
#if DEBUG >= 2
        std::cerr << "Initialize tvb computer..." << std::endl;
        std::cerr << "encoding_table = " << array1s_t(this->encoding_table)
                  << std::endl;
        std::cerr << "sizeof(encoding_table) = " << sizeof(this->encoding_table)
                  << std::endl;
#endif
    }
    // @}
    ...
    size_t receiver_sharedmem() const { return computer.receiver_sharedmem(); }
    // @}
#ifdef __CUDACC__
    ...
    void R(int d,
           int i,
           const cuda::vector_reference<sig>& r,
           cuda::vector_reference<real2>& ptable) const
    {
        const cuda::vector<sig>& tx = encoding_table.extract(
            cw_start(i) + d * cw_length(i), cw_length(i));
