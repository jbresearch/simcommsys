...
private:
    ...
    bool norm;  //!< Flag to indicate if metrics should be normalized between
                //!< time-steps
    typename qids<sig, real2>::metric_computer
...
public:
              ...
              const libcomm::qids<sig, real2>& chan)
    {
        ...
        // channel receiver
        computer =
            dynamic_cast<const typename qids<sig, real2>::metric_computer&>(
                chan.get_computer());
    }

    /*! \name Parameter getters */
    int get_mtau_min() const { return mtau_min; }
    int get_mtau_max() const { return mtau_max; }
    int get_m1_min() const { return m1_min; }
    int get_m1_max() const { return m1_max; }
    // @}
