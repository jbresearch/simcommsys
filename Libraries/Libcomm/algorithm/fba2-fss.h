...
private:
    ...
    array1vd_t
        app; //!< Copy of a-priori statistics, for local computation of gamma
    ...
    int Zmin; //!< The largest negative drift within a whole frame is \f$
              //!< m_\tau^{-} \f$
    int Zmax; //!< The largest positive drift within a whole frame is \f$
              //!< m_\tau^{+} \f$
              // @}
private:
     ...
     *        m2=inner loop
     */
    real& gamma_storage_entry(
        int d, int i, int m1, int delta1, int m2, int delta2) const
    {
        if (globalstore) {
    ...
    void normalize_alpha(int i) { normalize(alpha, i, Zmin, Zmax); }
    void normalize_beta(int i) { normalize(beta, i, Zmin, Zmax); }
    // decode functions - partial computations
    void
    work_gamma(const array1s_t& r, const array1vd_t& app, const int i) const
    {
        ...
        for (int m1 = Zmin; m1 <= Zmax; m1++) {
            ...
            const int length =
                std::min(cw_length(i) + std::min(Zmax - m1, cw_length(i)),
                         r.size() - start);
            ...
            for (int delta1 = 0; delta1 <= 1; delta1++) {
                ...
                for (int d = 0; d < q; d++) {
                    ...
                    for (int m2 = Zmin; m2 <= Zmax; m2++) {
                        gamma_storage_entry(d, i, m1, delta1, m2, 0) =
                            ptable0(m2 - Zmin);
                        gamma_storage_entry(d, i, m1, delta1, m2, 1) =
                            ptable1(m2 - Zmin);
                    }
                }
            }
