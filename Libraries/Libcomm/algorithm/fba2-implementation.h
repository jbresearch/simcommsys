...
template <class receiver_t,
          ...
          bool lazy,
          bool globalstore>
int fba2<receiver_t, sig, real, real2, thresholding, lazy, globalstore>::
    scale_calls = 0;
...
template <class receiver_t,
          ...
          bool lazy,
          bool globalstore>
real fba2<receiver_t, sig, real, real2, thresholding, lazy, globalstore>::
    get_threshold(const array2r_t& metric,
                  int row,
                  int col_min,
                  int col_max,
                  real factor,
...
void fba2<receiver_t, sig, real, real2, thresholding, lazy, globalstore>::
    work_alpha(const int i)
{
    // determine the strongest path at this point
    const real threshold =
        get_threshold(alpha, i - 1, mtau_min, mtau_max, th_inner, tp_states);
#if DEBUG >= 3
    int count = 0;
#endif
    for (int x1 = mtau_min; x1 <= mtau_max; x1++) {
        ...
        for (int x2 = x2min; x2 <= x2max; x2++) {
            // NOTE: we're repeating the loop on x2, so we need to increment
            // this
...
template <class receiver_t,
          ...
          bool globalstore>
void fba2<receiver_t, sig, real, real2, thresholding, lazy, globalstore>::
    work_message_app(array1vr_t& ptable, const int i) const
{
    // determine the strongest path at this point
    const real threshold =
        get_threshold(alpha, i, mtau_min, mtau_max, th_outer, tp_states);
    for (int d = 0; d < q; d++) {
#if DEBUG >= 3
        int count = 0;
#endif
...
template <class receiver_t,
          ...
          bool globalstore>
void fba2<receiver_t, sig, real, real2, thresholding, lazy, globalstore>::
    work_state_app(array1r_t& ptable, const int i) const
{
    assert(i >= 0 && i <= N);
    ...
    allocate()
{
    ...
    if (globalstore) {
         ...
         */
        gamma.global.resize(boost::extents[N][range(mtau_min, mtau_max + 1)][q]
                                          [range(mn_min, mn_max + 1)]);
         ...
         * deltax in [mn_min, mn_max]
         */
        gamma.local.resize(boost::extents[range(mtau_min, mtau_max + 1)][q]
                                         [range(mn_min, mn_max + 1)]);
    ...
    if (lazy) {
        if (globalstore) {
             ...
             * x in [mtau_min, mtau_max]
             */
            cached.global.resize(
                boost::extents[N][range(mtau_min, mtau_max + 1)]);
    ...
    if (globalstore) {
        std::cerr << "gamma = " << q << "×" << N << "×"
                  ...
                  << " = " << gamma.global.num_elements() << std::endl;
        if (lazy) {
        ...
        std::cerr << "gamma = " << q << "×" << mtau_max - mtau_min + 1 << "×"
                  << mn_max - mn_min + 1 << " = " << gamma.local.num_elements()
                  << std::endl;
        if (lazy) {
...
template <class receiver_t,
          ...
          bool globalstore>
void fba2<receiver_t, sig, real, real2, thresholding, lazy, globalstore>::
    work_gamma(const array1s_t& r, const array1vd_t& app)
{
    assert(initialised);
#ifndef NDEBUG
    if (app.size() == 0) {
...
template <class receiver_t,
          ...
          bool globalstore>
void fba2<receiver_t, sig, real, real2, thresholding, lazy, globalstore>::
    work_alpha_and_beta(const array1d_t& sof_prior, const array1d_t& eof_prior)
{
    assert(initialised);
...
template <class receiver_t,
          ...
          bool globalstore>
void fba2<receiver_t, sig, real, real2, thresholding, lazy, globalstore>::
    work_results(array1vr_t& ptable,
                 array1r_t& sof_post,
                 array1r_t& eof_post) const
{
    ...
    // ptable(i,d) = posterior prob. of having transmitted symbol 'd' at time
    // 'i'
...
template <class receiver_t,
          ...
          bool globalstore>
void fba2<receiver_t, sig, real, real2, thresholding, lazy, globalstore>::
    work_alpha(const array1d_t& sof_prior)
{
    assert(initialised);
...
template <class receiver_t,
          ...
          bool globalstore>
void fba2<receiver_t, sig, real, real2, thresholding, lazy, globalstore>::
    work_beta_and_results(const array1d_t& eof_prior,
                          array1vr_t& ptable,
                          array1r_t& sof_post,
                          array1r_t& eof_post)
{
           ...
           const int offset)
{
    ...
    collector.add_timer(sizeof(real) * (gamma.global.num_elements() +
    ...
    if (lazy) {
        const double usage =
            gamma_misses / double(N * (mtau_max - mtau_min + 1));
        const double reuse =
            gamma_calls / double(gamma_misses * q * (mn_max - mn_min + 1));
