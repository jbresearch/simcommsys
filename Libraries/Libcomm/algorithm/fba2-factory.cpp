...
std::shared_ptr<fba2_interface<sig, real, real2>>
fba2_factory<sig, real, real2>::get_instance(bool fss,
                                             bool thresholding,
                                             bool lazy,
                                             bool globalstore)
{
...
#define CONDITIONAL(r, args)                                                   \
    if (BOOST_PP_SEQ_ELEM(0, args) == thresholding &&                          \
        ...
        fba_ptr.reset(new FBA_TYPE<RECV_TYPE,                                  \
                                   sig,                                        \
                                   real,                                       \
                                   real2,                                      \
    ...
    if (!fss) {
        ...
        if (globalstore) {
            fba_ptr.reset(new fba2_fss<tvb_fss_receiver<sig, real, real2>,
                                       sig,
                                       real,
                                       real2,
                                       true>);
