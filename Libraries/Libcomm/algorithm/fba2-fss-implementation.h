...
fba2_fss<receiver_t, sig, real, real2, globalstore>::work_alpha(const int i)
{
    for (int m1 = Zmin; m1 <= Zmax; m1++) {
        for (int delta1 = 0; delta1 <= 1; delta1++) {
            ...
            const int m2min =
                std::max(Zmin, m1 - int(std::ceil(cw_length(i - 1) / 2.0)));
            for (int m2 = m2min; m2 <= m2max; m2++) {
                for (int delta2 = 0; delta2 <= 1; delta2++) {
                    // NOTE: we're repeating the loop on m2,delta2 so we need to
                    // increment this
                    real this_alpha = alpha[i][m2][delta2];
                    for (int d = 0; d < q; d++) {
                        real temp = prev_alpha;
                        temp *= gamma_storage_entry(
                            d, i - 1, m1, delta1, m2, delta2);
                        this_alpha += temp;
                    }
...
fba2_fss<receiver_t, sig, real, real2, globalstore>::work_beta(const int i)
{
    for (int m1 = Zmin; m1 <= Zmax; m1++) {
        for (int delta1 = 0; delta1 <= 1; delta1++) {
            ...
            const int m2max = Zmax;
            const int m2min =
                std::max(Zmin, m1 - int(std::ceil(cw_length(i) / 2.0)));
            for (int m2 = m2min; m2 <= m2max; m2++) {
                for (int delta2 = 0; delta2 <= 1; delta2++) {
                    ...
                    for (int d = 0; d < q; d++) {
                        real temp = next_beta;
                        temp *=
                            gamma_storage_entry(d, i, m1, delta1, m2, delta2);
                        this_beta += temp;
                    }
                }
            }
    ...
    for (int d = 0; d < q; d++) {
        ...
        for (int m1 = Zmin; m1 <= Zmax; m1++) {
            for (int delta1 = 0; delta1 <= 1; delta1++) {
                ...
                // Determine limits over this sequence
                // a) upper limit is the same as global limit
                // b) lower limit is bounded by ⌈n/2⌉ (one deletion per output
                // bit)
                const int m2max = Zmax;
                const int m2min =
                    std::max(Zmin, m1 - int(std::ceil(cw_length(i) / 2.0)));
...
fba2_fss<receiver_t, sig, real, real2, globalstore>::allocate()
{
    ...
    if (globalstore) {
         ...
         * d in [0, q-1]
         */
        gamma.global.resize(boost::extents[N][range(Zmin, Zmax + 1)][2][q]
                                          [range(Zmin, Zmax + 1)][2]);
         ...
         * d in [0, q-1]
         */
        gamma.local.resize(boost::extents[range(Zmin, Zmax + 1)][2][q]
                                         [range(Zmin, Zmax + 1)][2]);
    ...
    std::cerr << "alpha = " << N + 1 << "×" << Zmax - Zmin + 1
              << "×2 = " << alpha.num_elements() << std::endl;
    std::cerr << "beta = " << N + 1 << "×" << Zmax - Zmin + 1
              << "×2 = " << beta.num_elements() << std::endl;
    if (globalstore) {
        std::cerr << "gamma = " << q << "×" << N << "×" << Zmax - Zmin + 1
                  << "×2×" << Zmax - Zmin + 1
                  << "×2 = " << gamma.global.num_elements() << std::endl;
    } else {
        std::cerr << "gamma = " << q << "×" << Zmax - Zmin + 1 << "×2×"
                  << Zmax - Zmin + 1 << "×2 = " << gamma.local.num_elements()
                  << std::endl;
    }
#endif
}
                ...
                for (int delta1 = 0; delta1 <= 1; delta1++) {
                    ...
                    for (int delta2 = 0; delta2 <= 1; delta2++) {
                        for (int m2 = Zmin; m2 <= Zmax; m2++) {
                            sout << '\t'
                                 << gamma.global[i][m1][delta1][d][m2][delta2];
                        }
                        sout << std::endl;
                    }
                }
    ...
    const array1d_t& sof_prior, const array1d_t& eof_prior)
{
    ...
    // initialise arrays:
    // NOTE: technically unnecessary for beta, as we initialize this_beta for
    // every value
    alpha = real(0);
    beta = real(0);
        ...
        computer)
{
    ...
    if (initialised && (N != this->N || q != this->q ||
